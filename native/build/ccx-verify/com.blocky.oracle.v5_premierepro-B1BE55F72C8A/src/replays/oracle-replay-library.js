...
(function exposeOracleReplayLibrary(globalScope, factory) {
  ...
  if (!schema || !migrations) throw new Error("Blocky Studios replay data dependencies did not load.");
  ...
  function createReplayMetadataExport(state, options = {}) {
    ...
    if (!validation.ok) {
      const error = /** @type {TypeError & { code?: string, diagnostics?: Array<object>, cause?: unknown }} */ (
        new TypeError(`Replay metadata export requires valid Blocky Studios v3 state: ${validation.errors.join("; ")}`)
      );
  ...
  function normalizeReplayMetadataImport(payload, options = {}) {
    ...
    if (rawImageFindings.length > 0) {
      return metadataImportFailure(
        ...
        rawImageFindings.map((finding) => ({
          ...
          path: finding.path,
          message: `Raw thumbnail or image bytes at ${finding.path} are not part of durable Blocky Studios metadata.`,
        })),
      );
    }
    ...
    if (isCurrentState) {
      ...
      if (!validation.ok) {
        return metadataImportFailure(
          ...
          "Replay metadata contains an invalid Blocky Studios v3 state snapshot.",
    ...
    if (!migrated || !migrated.sourceValid) {
      return metadataImportFailure(
        "INVALID_METADATA_STATE",
        "Replay metadata does not contain a supported Blocky Studios state snapshot.",
    ...
    if (!migratedValidation.ok) {
      return metadataImportFailure(
        "INVALID_METADATA_STATE",
        "Replay metadata migration did not produce valid Blocky Studios v3 state.",
  ...
  function replayToPresentation(record, runtime = {}) {
    ...
    return {
      ...
      fileIdentity: record.fileIdentity ? { ...record.fileIdentity } : null,
      fileSize: record.fileSize,
      modifiedAt: record.modifiedAt,
      thumbnail: record.thumbnailCacheKey
        ? `plugin-data:/oracle-thumbnail-v2-${record.thumbnailCacheKey}.jpg`
        : "",
  ...
  function replaySearchDocument(record, state) {
    const legacy = record.legacy || {};
    const exported = new Date(record.exportedAt || "");
    const monthIndex = exported.getUTCMonth();
    const shortMonths = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
    const longMonths = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
    const searchableDate = Number.isFinite(exported.getTime())
      ? `${shortMonths[monthIndex]} ${exported.getUTCDate()} ${exported.getUTCFullYear()} ${longMonths[monthIndex]} ${exported.getUTCDate()} ${exported.getUTCFullYear()}`
      : "";
    const collectionNames = (record.collectionIds || [])
      .map((id) => state && state.collectionsById && state.collectionsById[id])
      .filter(Boolean)
    ...
    return [
      ...
      legacy.resolution,
      legacy.fps,
      legacy.fps === null || legacy.fps === undefined ? "" : `${legacy.fps} FPS`,
      record.durationMs,
      formatReplayDuration(record.durationMs),
      record.exportedAt,
      searchableDate,
      record.notes,
  ...
  function selectReplayIds(state, options = {}) {
    ...
    const records = Object.values(state && state.replaysById || {}).filter((record) => {
      const unresolved = record.missingState === "missing" || record.missingState === "unknown";
      if (view === "favorites" && !record.favorite) return false;
      if (view === "archived" && record.archiveState !== "archived" && !unresolved) return false;
      if (view === "missing" && (!unresolved || record.archiveState === "archived")) return false;
      if (view !== "archived" && view !== "missing" && (record.archiveState === "archived" || unresolved)) return false;
      ...
      return !query || replaySearchDocument(record, state).includes(query);
    });
  ...
  class ReplayLibraryStore {
    ...
    hydrate(value) {
      ...
      if (!result.sourceValid) throw new Error("Blocky Studios replay state failed v3 migration validation.");
  ...
  class OracleStateRepository {
    ...
    save(state) {
      if (this.destroyed) return Promise.reject(new Error("Blocky Studios state repository is closed."));
    ...
    async writeSnapshot(snapshot) {
      if (!this.fs && !this.atomicWriter) throw new Error("Blocky Studios v3 state storage is unavailable.");
      ...
      if (
        ...
        throw new Error("Blocky Studios state storage lacks atomic replace primitives.");
      }
      ...
      if (!staged.sourceValid || !staged.complete || stableJson(staged.state) !== stableJson(snapshot)) {
        await unlinkIfPresent(fs, STATE_TEMP_URL);
        throw new Error("Blocky Studios rejected an incomplete staged v3 snapshot.");
      }
      ...
      try {
        ...
        if (!committed.sourceValid || stableJson(committed.state) !== stableJson(snapshot)) {
          throw new Error("Blocky Studios could not verify the committed v3 snapshot.");
        }
      } catch (error) {
  ...
  class ThumbnailCache {
    ...
    url(key) {
      return key ? `plugin-data:/oracle-thumbnail-v2-${key}.jpg` : "";
    }
    ...
    adoptNative(record, nativeResult, options = {}) {
      const generation = this.generation;
      const operation = this.createOperation(record);
      return this.queue.submit(async () => {
        try {
          ...
          const key = thumbnailCacheKey(record, options);
          if (
            !nativeResult || nativeResult.thumbnailReady !== true ||
            String(nativeResult.cacheKey || "").toLowerCase() !== key.toLowerCase()
          ) {
            throw new Error("Native thumbnail cache identity did not match this replay.");
          }
          const output = validateJpegBytes(await this.fs.readFile(this.url(key)), variant);
          if (output.bytes.byteLength > this.cacheLimitBytes(options.limitMb)) {
            throw new Error("Thumbnail variant exceeds the configured cache limit.");
          }
          this.assertCurrent(generation, operation);
          return this.mutate(async () => {
            ...
            return {
              ...
              width: variant.width,
              height: variant.height,
              sourceWidth: Math.max(0, Number(nativeResult.sourceWidth) || variant.width),
              sourceHeight: Math.max(0, Number(nativeResult.sourceHeight) || variant.height),
              reused: nativeResult.thumbnailReused === true,
              native: true,
            };
          });
        } finally {
