...
const ORACLE_PLUGIN_VERSION = "2.0.16";
...
class SmoothWheelScroller {
  constructor(viewport) {
    ...
    this.eventRoot = viewport && typeof viewport.closest === "function"
      ? viewport.closest("[data-oracle-panel-root], .oracle-panel") || viewport
      : viewport;
  ...
  start() {
    ...
    try {
      this.eventRoot.addEventListener("wheel", this.onWheel, this.listenerOptions);
    } catch (error) {
      this.listenerOptions = true;
      this.eventRoot.addEventListener("wheel", this.onWheel, true);
    }
  ...
  onWheel(event) {
    ...
    if (event.target && typeof this.viewport.contains === "function" && !this.viewport.contains(event.target)) return;
  ...
  destroy() {
    ...
    this.started = false;
    this.cancelAnimation();
    this.eventRoot.removeEventListener("wheel", this.onWheel, this.listenerOptions);
