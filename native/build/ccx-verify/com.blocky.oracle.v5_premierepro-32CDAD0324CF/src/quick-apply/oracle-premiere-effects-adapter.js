...
(function exposeOraclePremiereEffectsAdapter(globalScope, factory) {
  ...
  const api = factory(effectIndexApi);
  if (typeof module === "object" && module && module.exports) module.exports = api;
  if (globalScope) Reflect.set(globalScope, "OraclePremiereEffectsAdapter", api);
})(typeof window !== "undefined" ? window : null, function createOraclePremiereEffectsAdapterApi(effectIndexApi) {
  ...
  const REASON_MESSAGES = Object.freeze({
    ...
    [REASON.HOST_VERSION_UNAVAILABLE]: "Premiere's host version is unavailable, so Oracle will not create an unversioned effect cache.",
    ...
    [REASON.AMBIGUOUS_AUDIO_DISPLAY_NAME]: "Premiere exposes more than one installed audio effect with this display name, but its documented factory accepts only the display name. Oracle will not guess which duplicate to apply.",
    ...
    [REASON.INVALID_RECIPE]: "The Oracle Recipe is invalid or exceeds supported limits.",
    [REASON.RECIPE_PREMIERE_VERSION_UNSUPPORTED]: "This Oracle Recipe requires a newer Premiere version.",
    [REASON.RECIPE_EFFECT_INDEX_VERSION_UNSUPPORTED]: "This Oracle Recipe targets a different Premiere effect-index version.",
    [REASON.PARAMETER_UNSUPPORTED]: "A recipe parameter cannot be set through Premiere's documented action API.",
    [REASON.PARAMETER_METADATA_UNAVAILABLE]: "Premiere 26.3 does not expose documented generic parameter names, ranges, or enum options for effect discovery. Oracle will not invent them.",
    [REASON.TIME_VARYING_PARAMETER_UNSUPPORTED]: "Oracle Recipes do not overwrite an already keyframed parameter.",
    ...
    [REASON.PARTIAL_FAILURE]: "The effect stack was added, but a later parameter stage failed. Oracle has reported the committed boundary exactly.",
  });
  ...
  async function readProjectItemId(api, projectItem) {
    ...
    try {
      if (typeof projectItem.getId === "function") {
        const id = await Promise.resolve(projectItem.getId());
      ...
      if (api && api.UniqueSerializeable && typeof api.UniqueSerializeable.cast === "function") {
        ...
        if (serializable && typeof serializable.getUniqueID === "function") return guidText(serializable.getUniqueID());
      }
  ...
  async function readComponentChain(chain, maximum) {
    ...
    const count = Number(await Promise.resolve(chain.getComponentCount()));
  ...
  class PremiereQuickApplyAdapter {
    ...
    async buildHostEffectIndex() {
      ...
      try {
        [videoDisplayNames, videoMatchNames, audioDisplayNames] = await Promise.all([
          videoFactory.getDisplayNames(),
          videoFactory.getMatchNames(),
          audioFactory.getDisplayNames(),
        ]);
      } catch (error) {
    ...
    publish(snapshot) {
      ...
      for (const listener of Array.from(this.subscribers)) {
        try { listener(snapshot); } catch (error) {
          if (this.logger && typeof this.logger.error === "function") this.logger.error("[Oracle Quick Apply] Snapshot subscriber failed.", error);
        }
      }
    }
    ...
    unbindSequenceEvents() {
      ...
      if (manager && typeof manager.removeEventListener === "function") {
        for (const entry of this.observedSequenceListeners.splice(0)) {
          try { manager.removeEventListener(entry.sequence, entry.eventName, this.hostEventListener); } catch (error) { /* best effort cleanup */ }
        }
    ...
    async createEffectCandidate(effect, ref) {
      ...
      try {
        if (effect.type === "video") {
          ...
          component = await factory.createComponent(effect.matchName);
        } else {
          ...
          component = await factory.createComponentByDisplayName(effect.displayName, ref.item);
        }
      } catch (error) {
    ...
    preflightAppendComponent(project, ref, component) {
      ...
      try {
        project.lockedAccess(() => {
          try { action = ref.chain.createAppendComponentAction(component); } catch (error) {
            callbackError = error;
            throw error;
          }
        });
      ...
      try {
        project.lockedAccess(() => {
          try {
            ...
            const keyframe = param.createKeyframe(createHostParameterValue(this.api, parameter.value));
    ...
    async planEffectApplication(effectRef, options = {}) {
      ...
      return this.issuePlan("oracle-effect-application-plan", inspection, {
        ...
        undoString: cleanText(options.undoString, 128) || `Oracle Quick Apply: ${effect.displayName}`,
        effect: effectPublicCopy(effect),
        applyOnce: options.applyOnce === true,
    ...
    async planRecipeApplication(recipeValue, options = {}) {
      ...
      return this.issuePlan("oracle-recipe-application-plan", inspection, {
        ...
        projectGuid: inspection.snapshot.project && inspection.snapshot.project.guid,
        sequenceGuid: inspection.snapshot.sequence && inspection.snapshot.sequence.guid,
        undoString: cleanText(options.undoString, 128) || `Oracle Recipe: ${recipe.name}`,
        parameterUndoString: cleanText(options.parameterUndoString, 128) || `Oracle Recipe Parameters: ${recipe.name}`,
    ...
    appendActionFactory(target) {
      return () => {
        ...
        let action;
        try { action = target.ref.chain.createAppendComponentAction(target.component); } catch (error) {
          ...
          try {
            ...
            transactionResult = project.executeTransaction((compoundAction) => {
              ...
              for (const action of actions) {
                if (compoundAction.addAction(action) !== true) throw new QuickApplyAdapterError(REASON.ACTION_REJECTED);
              }
            }, undoString);
          } catch (error) {
            callbackError = error;
    ...
    async verifyParameterReadback(records) {
      for (const record of records) {
        ...
        const keyframe = await record.param.getStartValue();
