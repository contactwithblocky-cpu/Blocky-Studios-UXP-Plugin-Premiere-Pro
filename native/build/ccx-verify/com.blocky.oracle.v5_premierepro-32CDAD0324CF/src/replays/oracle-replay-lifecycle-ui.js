  ...
  function renderSmartRuleValueControls(ruleRow, rule = {}) {
    ...
    values.append(field(
      ...
      textInput("smartRuleTextValue", rule.value, {
        ...
        placeholder: fieldName === "root" ? "D:\\Oracle Replays" : "",
      }),
    ));
  }
  ...
  class ReplayLifecycleDialogController {
    ...
    render() {
      ...
      if (this.mode === "rename-display") {
        ...
        body.append(
          field("Display name", textInput("displayName", first && first.title, { maxLength: 240 }), "This changes Oracle metadata only. The source filename stays unchanged."),
        ...
        body.append(
          element("p", "replay-lifecycle-warning", "Oracle will revalidate the exact file identity immediately before the rename. The video extension is preserved."),
        ...
        body.append(
          ...
          field("Notes", textInput("notes", first && first.notes, { multiline: true, maxLength: 16000 }), "Notes are searchable inside Oracle."),
        );
        ...
        body.append(
          element("p", "replay-lifecycle-warning", action === "restore"
            ? "Restoring returns these records to the active Oracle library. It does not modify source media or Premiere ProjectItems."
        ...
        if (this.context.operationResult) {
          ...
          body.append(
            element("p", "replay-lifecycle-warning", "The default action only archives the Oracle library record. It does not remove media, sidecars, folders, or Premiere ProjectItems."),
    ...
    renderRelinkResult() {
      ...
      if (needsConfirmation) {
        if (!ambiguousAssignments.length || !auditComplete) {
          result.append(element("p", "replay-lifecycle-blocker", "Oracle cannot apply this ambiguous relink until every affected replay includes its source, candidate path, numeric score, and ambiguity reason."));
        } else {
