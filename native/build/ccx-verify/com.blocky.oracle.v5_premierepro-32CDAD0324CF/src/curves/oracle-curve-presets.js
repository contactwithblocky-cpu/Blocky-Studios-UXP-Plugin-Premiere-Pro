...
(function exposeOracleCurvePresets(globalScope, factory) {
  ...
  if (!curveMath) throw new Error("Oracle curve math did not load before curve presets.");
  ...
  const MAX_IMPORT_CHARACTERS = 1000000;
  ...
  function validatePresetLibrary(input) {
    ...
    if (input && input.schema && input.schema !== PRESET_LIBRARY_SCHEMA) errors.push("Library schema is not an Oracle curve preset library.");
    if (input && Number(input.version) > PRESET_LIBRARY_VERSION) errors.push("Library version is newer than this Oracle build.");
  ...
  function createUserPreset(input, draft, options = {}) {
    return commitLibrary(input, options, (library) => {
      if (library.presets.length >= MAX_USER_PRESETS) {
        return failure("PRESET_LIMIT", `Oracle supports at most ${MAX_USER_PRESETS} user curve presets.`);
      }
  ...
  function createFolder(input, name, options = {}) {
    return commitLibrary(input, options, (library) => {
      if (library.folders.length >= MAX_FOLDERS) return failure("FOLDER_LIMIT", `Oracle supports at most ${MAX_FOLDERS} curve folders.`);
  ...
  function exportPresetLibrary(input, options = {}) {
    if (isPremierePresetFilename(options.filename) || cleanString(options.format, 32).toLocaleLowerCase("en-US") === "prfpset") {
      return failure(
        "UNSUPPORTED_FORMAT",
        "Oracle curve libraries use versioned JSON and cannot be exported as Premiere preset files.",
      );
    }
    return {
      ok: true,
      filename: cleanString(options.filename, 260) || "oracle-curve-presets.json",
  ...
  function parseImportSource(source) {
    ...
    if (source.length > MAX_IMPORT_CHARACTERS) {
      return failure("IMPORT_TOO_LARGE", `Curve preset JSON exceeds ${MAX_IMPORT_CHARACTERS} characters.`);
    }
    try {
  ...
  function importPresetLibrary(source, currentInput, options = {}) {
    if (isPremierePresetFilename(options.filename)) {
      return failure("UNSUPPORTED_FORMAT", "Premiere preset files are not Oracle curve libraries.");
    }
    ...
    if (
      ...
      return failure("INVALID_SCHEMA", "The selected JSON is not an Oracle curve preset library.");
    }
