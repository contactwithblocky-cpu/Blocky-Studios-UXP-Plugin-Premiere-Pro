  ...
  async scanAndSchedule() {
    ...
    try {
      for (const state of Array.from(this.directories.values())) {
        await this.scanDirectory(state);
        await yieldToHost();
      }
    } catch (error) {
...
class PremiereSourceMonitorViewerAdapter {
  ...
  async open(replay) {
    ...
    const opened = await this.api.SourceMonitor.openFilePath(absolutePath);
    if (opened === false) {
      throw new UserFacingError(
        "Premiere could not open this replay in Source Monitor.",
        true,
...
class ReplayGridView {
  ...
  renderVirtualWindow() {
    ...
    for (let index = range.start; index < range.end; index += 1) {
      const row = this.layoutRows[index];
      if (row.kind === "header") {
        nodes.push(this.createDateHeading(row.label));
        continue;
      }
  ...
  prefersReducedMotion() {
    const preference = oracleWindow.oracleWorkspacePreferences &&
      ...
      oracleWindow.oracleWorkspacePreferences.appearance.motion;
    if (preference === "reduce") return true;
    if (preference === "allow") return false;
...
class CurvePresetDomainStore {
  constructor(store, presetApi, persist) {
    this.store = store;
...
class OraclePanelController {
  constructor() {
    ...
    this.theme = new preferencesApi.OraclePreferencesController(this.elements, {
      ...
      getDiagnosticsSummary: () => typeof oracleDiagnostics !== "undefined" && oracleDiagnostics && typeof oracleDiagnostics.summary === "function"
        ? oracleDiagnostics.summary({ limit: 20 })
        : null,
  ...
  initializeQuickApply() {
    ...
    this.quickApplyEffectIndexStore = new QuickApplyEffectIndexStore(this.store, persistQuickApplyState);
  ...
  async releaseReplayMutationHandles(replayIds) {
    ...
    if (this.viewer && typeof this.viewer.releaseReplayIds === "function") {
      await this.viewer.releaseReplayIds(ids);
    }
    for (const mount of Array.from(this.dedicatedMounts || [])) {
      if (mount && mount.viewer && typeof mount.viewer.releaseReplayIds === "function") {
        await mount.viewer.releaseReplayIds(ids);
      }
    }
  ...
  pollNativeDirectoryWatch() {
    if (this.destroyed || !nativeDragAddon || typeof nativeDragAddon.pollDirectoryWatchEvents !== "function") return;
    let events;
    try {
    ...
    for (const event of Array.isArray(events) ? events : []) {
      if (event.kind === "renamed" && event.sameVolumeIdentityMatched && event.recordId) {
        void this.reconcileNativeWatcherRename(event);
  ...
  scheduleNativeMissingVerification(replayId, observedPath) {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        if (this.viewer && typeof this.viewer.releaseReplayIds === "function") {
          try {
            await this.viewer.releaseReplayIds([id]);
          } catch (releaseError) {
            this.showToast("Oracle could not release the missing replay from Source Monitor.", "error");
          }
        }
  ...
  scheduleReplayProcessing(replayId, origin) {
    if (this.destroyed || !replayId) return;
    ...
    const processingTask = this.metadataQueue.submit(async () => {
      if (this.destroyed || this.processingGenerations.get(replayId) !== generation) return;
      ...
      if (uxpFs && typeof uxpFs.lstat === "function") {
        ...
        for (let attempt = 0; attempt < 3; attempt += 1) {
          ...
          const declaredSizeMatches = record.fileSize !== null && Number(record.fileSize) === fileSize;
          const declaredTimeMatches = !record.modifiedAt || !modifiedAt || record.modifiedAt === modifiedAt;
          if ((declaredSizeMatches && declaredTimeMatches) ||
      ...
      try {
        if (missing && this.viewer && typeof this.viewer.releaseReplayIds === "function") {
          await this.viewer.releaseReplayIds([replayId]);
        }
        this.store.updateById(replayId, {
