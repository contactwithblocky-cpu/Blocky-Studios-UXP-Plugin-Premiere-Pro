...
(function exposeOracleQuickApplyWorkspace(globalScope, factory) {
...
})(typeof window !== "undefined" ? window : null, function createOracleQuickApplyWorkspaceApi(defaultDomainApi, globalScope) {
  ...
  function normalizeElements(value = {}) {
    ...
    for (const [name, aliases] of Object.entries(ELEMENT_ALIASES)) {
      output[name] = null;
      // The shared main-panel map contains generic replay keys such as
      // `empty` as well as Quick Apply's canonical IDs. Prefer the canonical
      // workspace key so a generic alias can never bind this controller to a
      // sibling workspace when both are present. Generic aliases remain as a
      // compatibility path for dedicated mounts and focused harnesses.
      const canonical = DOM_IDS[name];
      const candidates = [canonical, ...aliases.filter((alias) => alias !== canonical)];
  ...
  class QuickApplyWorkspaceController {
    ...
    start() {
      ...
      if (typeof this.domain.setLifecycle === "function") {
        this.domain.setLifecycle({ visible: this.visible, active: this.active });
      } else {
    ...
    setVisible(value) {
      this.setLifecycle({ visible: value, active: this.active });
    }

    setActive(value) {
      this.setLifecycle({ visible: this.visible, active: value });
    }

    setLifecycle(value = {}) {
      if (this.destroyed) return false;
      const nextVisible = Boolean(value.visible);
      const nextActive = Boolean(value.active);
      if (nextVisible === this.visible && nextActive === this.active) return false;
      ...
      if (!this.visible || !this.active) {
        ...
        if (this.elements.empty) {
          ...
          this.elements.empty.textContent = snapshot.query
            ? "No supported effect or Blocky Studios Recipe matches this search."
    ...
    setScope(scope, trigger = "interaction-or-controller") {
      const nextScope = cleanText(scope, 32);
      const previousScope = cleanText(this.snapshot && this.snapshot.scope, 32);
      this.domain.setScope(nextScope);
      const telemetry = globalScope && Reflect.get(globalScope, "oraclePlatformTelemetry");
      if (previousScope !== nextScope && telemetry && typeof telemetry.tabSwitch === "function") {
        const root = this.elements.root && this.elements.root.closest
          ? this.elements.root.closest("[data-oracle-panel-root], .oracle-panel") || this.elements.root
          : this.elements.root;
        telemetry.tabSwitch({
          root,
          panelId: root && root.dataset && root.dataset.oraclePanelRoot || "oracleQuickApplyPanel",
          group: "quick-apply-scope",
          from: previousScope,
          to: nextScope,
          trigger,
        });
    ...
    onScopeKeyDown(event, scopeControl) {
      ...
      this.setScope(scopeControl.dataset.quickApplyScope, "keyboard");
      return true;
    }
    ...
    renderResults(snapshot) {
      ...
      const groups = snapshot.grouping !== "none" && Array.isArray(snapshot.targetGroups)
        ? snapshot.targetGroups.filter((group) => group && Array.isArray(group.targets) && group.targets.length)
        : [];
      const signature = JSON.stringify({
        grouping: snapshot.grouping,
        groups: groups.map((group) => [group.id, group.label, group.targets.map((target) => target.id)]),
        targets: targets.map((target) => [
          target.id,
          target.name,
          target.mediaType,
          target.kind,
          target.categoryAvailable,
          target.category,
          target.compatible,
          target.favorite,
          target.recent,
          target.compatibility && target.compatibility.reason,
          target.recipe && target.recipe.steps && target.recipe.steps.length,
        ]),
      });
      if (signature === this.resultStructureSignature) {
        this.syncRenderedResults(snapshot);
        return;
      }
      this.resultStructureSignature = signature;
      ...
      for (const group of groups) {
        ...
        container.className = "quick-apply-result-group";
        container.setAttribute("role", "group");
        const label = cleanText(group.label, 256) || "Quick Apply results";
        container.setAttribute("aria-label", `${label}, ${group.targets.length} result${group.targets.length === 1 ? "" : "s"}`);
        const heading = this.document.createElement("div");
        heading.className = "quick-apply-result-group__heading";
        heading.setAttribute("aria-hidden", "true");
        heading.textContent = label;
        container.appendChild(heading);
        for (const target of group.targets) {
          container.appendChild(this.createResult(target, target.id === snapshot.selectedTargetId, snapshot.pendingTargetIds && snapshot.pendingTargetIds.includes(target.id)));
        }
        list.appendChild(container);
      }
    }

    syncRenderedResults(snapshot) {
      const pendingIds = new Set(Array.isArray(snapshot.pendingTargetIds) ? snapshot.pendingTargetIds : []);
      const targets = new Map((Array.isArray(snapshot.targets) ? snapshot.targets : []).map((target) => [target.id, target]));
      for (const [id, nodes] of this.resultNodes) {
        const target = targets.get(id);
        if (!target || !nodes || !nodes.control) continue;
        const selected = id === snapshot.selectedTargetId;
        const pending = pendingIds.has(id);
        nodes.control.classList.toggle("is-selected", selected);
        nodes.control.classList.toggle("is-incompatible", !target.compatible);
        nodes.control.setAttribute("aria-selected", selected ? "true" : "false");
        nodes.control.setAttribute("aria-disabled", target.compatible ? "false" : "true");
        nodes.control.tabIndex = selected ? 0 : -1;
        nodes.control.title = pending ? `${target.name} is being applied.` : target.compatibility.reason || target.name;
        if (nodes.compatibility) {
          nodes.compatibility.textContent = pending
            ? "Applying…"
            : target.compatible ? target.compatibility.reason || "Compatible" : target.compatibility.reason || "Incompatible";
        }
      }
    }
    ...
    createResult(target, selected, pending) {
      ...
      const compatibility = appendTextElement(this.document, details, "span", `quick-apply-result__compatibility${target.compatible ? " is-compatible" : " is-incompatible"}`, pending ? "Applying…" : target.compatible ? target.compatibility.reason || "Compatible" : target.compatibility.reason || "Incompatible");
      control.appendChild(details);
      control.title = pending ? `${target.name} is being applied.` : target.compatibility.reason || target.name;
      this.resultNodes.set(target.id, { control, compatibility });
      return control;
    }
    ...
    renderActions(snapshot) {
      ...
      const noSelectionReason = "Choose a supported effect or Blocky Studios Recipe first.";
      ...
      setDisabled(this.elements.recipeAddEffect, !effectSelected, "Choose a supported effect to add it to a Blocky Studios Recipe.");
      setDisabled(this.elements.recipeRename, !recipeSelected, "Choose a Blocky Studios Recipe to rename it.");
      setDisabled(this.elements.recipeDuplicate, !recipeSelected, "Choose a Blocky Studios Recipe to duplicate it.");
      setDisabled(this.elements.recipeDelete, !recipeSelected, "Choose a Blocky Studios Recipe to delete it.");
      const recipeIndex = recipeSelected ? snapshot.library.recipes.findIndex((entry) => entry.id === target.recipeId) : -1;
      setDisabled(this.elements.recipeMoveUp, recipeIndex <= 0, recipeSelected ? "This Blocky Studios Recipe is already first." : "Choose a Blocky Studios Recipe to reorder it.");
      setDisabled(this.elements.recipeMoveDown, recipeIndex < 0 || recipeIndex >= snapshot.library.recipes.length - 1, recipeSelected ? "This Blocky Studios Recipe is already last." : "Choose a Blocky Studios Recipe to reorder it.");
      setDisabled(this.elements.recipeExport, !snapshot.library.recipes.length || !this.exportRecipeFile, !this.exportRecipeFile ? "Recipe export needs the UXP file picker integration." : "No Blocky Studios Recipes exist yet.");
    ...
    async onClick(event) {
      ...
      if (scopeControl) {
        ...
        this.setScope(scopeControl.dataset.quickApplyScope, "pointer");
        this.scheduleSearchFocus();
        return;
      }
    ...
    onInput(event) {
      if (event.target === this.elements.search) {
        ...
        const telemetry = globalScope && Reflect.get(globalScope, "oraclePlatformTelemetry");
        if (telemetry && typeof telemetry.interaction === "function") {
          const root = this.elements.root && this.elements.root.closest
            ? this.elements.root.closest("[data-oracle-panel-root], .oracle-panel") || this.elements.root
            : this.elements.root;
          telemetry.interaction({ root, panelId: root && root.dataset && root.dataset.oraclePanelRoot || "oracleQuickApplyPanel", name: "quick-apply-search", action: "input", state: String(event.target.value || "").length });
        }
        return;
      }
    ...
    async onKeyDown(event) {
      ...
      if (this.recipeOpen) {
        const catalogItem = closest(event, "[data-recipe-catalog-effect-id]");
        if (catalogItem && ["Enter", " ", "Spacebar"].includes(event.key)) {
          event.preventDefault();
          this.addEffectToDraft(catalogItem.dataset.recipeCatalogEffectId);
        }
        return;
      }
      const scopeControl = closest(event, "[data-quick-apply-scope]");
    ...
    async renameSelectedRecipe() {
      ...
      const name = await this.requestRecipeName({ mode: "rename", title: "Rename Blocky Studios Recipe", value: recipe.name, recipe: clone(recipe) });
      if (!cleanText(name, 256)) return;
      const result = await this.domain.renameRecipe(recipe.id, name);
      this.reportRecipeResult(result, "Blocky Studios Recipe renamed.");
    }

    async duplicateSelectedRecipe() {
      const recipe = this.selectedRecipe();
      if (!recipe || !this.requestRecipeName) return;
      const name = await this.requestRecipeName({ mode: "duplicate", title: "Duplicate Blocky Studios Recipe", value: `${recipe.name} Copy`, recipe: clone(recipe) });
      if (!cleanText(name, 256)) return;
      const result = await this.domain.duplicateRecipe(recipe.id, name);
      this.reportRecipeResult(result, "Blocky Studios Recipe duplicated.");
    }
    ...
    async moveSelectedRecipe(delta) {
      ...
      this.reportRecipeResult(result, result.unchanged ? "Blocky Studios Recipe order is unchanged." : "Blocky Studios Recipe reordered.");
    }
    ...
    async deleteSelectedRecipe() {
      ...
      const confirmed = await this.confirmRecipeAction({ mode: "delete", title: "Delete Blocky Studios Recipe", message: `Delete “${recipe.name}”? This cannot be undone inside Blocky Studios.`, recipe: clone(recipe) });
      if (!confirmed) return;
      const result = await this.domain.deleteRecipe(recipe.id);
      this.reportRecipeResult(result, "Blocky Studios Recipe deleted.");
    }

    async importRecipes() {
      if (!this.importRecipeFile) return;
      try {
        const value = await this.importRecipeFile({ extensions: ["json"], label: "Import Blocky Studios Recipes" });
        if (!value) return;
        const text = typeof value === "string" ? value : value.text;
        const result = await this.domain.importRecipes(text, { filename: typeof value === "object" ? value.filename : "" });
        this.reportRecipeResult(result, result.message || `${result.imported} Blocky Studios Recipes imported.`);
      } catch (error) {
        this.reportRecipeResult({ ok: false, message: cleanText(error && error.message, 2048) || "Blocky Studios Recipe import failed." });
      }
    }
    ...
    async exportRecipes() {
      ...
      try {
        ...
        await this.exportRecipeFile({ text, suggestedName: "Blocky-Studios-Recipes.json", mimeType: "application/json" });
        this.reportRecipeResult({ ok: true }, "Blocky Studios Recipes exported.");
      } catch (error) {
        this.reportRecipeResult({ ok: false, message: cleanText(error && error.message, 2048) || "Blocky Studios Recipe export failed." });
      }
    }

    reportRecipeResult(result, successMessage = "Blocky Studios Recipe updated.") {
      const message = result && result.ok ? cleanText(result.message, 2048) || successMessage : cleanText(result && result.message, 2048) || "Blocky Studios Recipe action failed.";
    ...
    renderRecipeEditor() {
      if (!this.recipeOpen || !this.recipeDraft) return;
      if (this.elements.recipeEditorTitle) this.elements.recipeEditorTitle.textContent = this.recipeDraft.id ? "Edit Blocky Studios Recipe" : "Create Blocky Studios Recipe";
    ...
    renderRecipeStack() {
      ...
      this.recipeDraft.steps.forEach((step, index) => {
        ...
        for (const [action, label, title, disabled] of [
          ["up", "Earlier", "Move effect earlier", index === 0],
          ["down", "Later", "Move effect later", index === this.recipeDraft.steps.length - 1],
          ["remove", "Remove", "Remove effect", false],
        ]) {
          const button = this.document.createElement("button");
          button.type = "button";
          button.className = action === "remove"
            ? "oracle-button oracle-button--danger"
            : "oracle-button oracle-button--quiet";
          button.dataset.recipeStepAction = action;
          button.dataset.recipeStepIndex = String(index);
          button.dataset.icon = action === "up" ? "move-earlier" : action === "down" ? "move-later" : "remove";
    ...
    renderRecipeCatalog() {
      ...
      for (const effect of effects) {
        ...
        item.className = "quick-apply-recipe-catalog__item";
        item.dataset.recipeCatalogEffectId = effect.id;
        item.setAttribute("role", "button");
        item.tabIndex = 0;
        appendTextElement(this.document, item, "span", `quick-apply-result__type quick-apply-result__type--${effect.mediaType}`, effect.mediaType === "audio" ? "Audio" : "Video");
        appendTextElement(this.document, item, "strong", "", effect.displayName);
        appendTextElement(this.document, item, "small", "", effect.categoryAvailable ? effect.category : "Category unavailable from Premiere");
    ...
    async saveRecipeEditor() {
      ...
      if (!name && this.requestRecipeName) name = cleanText(await this.requestRecipeName({ mode: "create", title: "Name Blocky Studios Recipe", value: "New Blocky Studios Recipe", recipe: clone(this.recipeDraft) }), 256);
      if (!name) {
        this.setRecipeEditorError("Enter a name for this Blocky Studios Recipe.");
      ...
      if (!result.ok) {
        this.setRecipeEditorError(result.message || "Blocky Studios Recipe could not be saved.");
        return;
      }
      this.closeRecipeEditor(false);
      this.reportRecipeResult(result, "Blocky Studios Recipe saved.");
      this.scheduleSearchFocus();
    }
