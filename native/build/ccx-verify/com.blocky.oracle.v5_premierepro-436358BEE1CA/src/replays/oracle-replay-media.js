...
(function exposeOracleReplayMedia(globalScope, factory) {
  ...
  if (!library || typeof library.thumbnailCacheKey !== "function") {
    throw new Error("Oracle replay media requires the replay library module.");
  }
  ...
  class ReplayMediaPipeline {
    ...
    prepare(record, options = {}) {
      if (!this.isAvailable()) {
        return Promise.reject(createMediaError(
          ...
          "Oracle's local replay preview service is unavailable.",
        ));
      }
    ...
    async run(record, cacheKey, variant, includePreview, generation) {
      ...
      try {
        ...
        throw createMediaError(error, "Oracle could not start local replay preview preparation.");
      }
      ...
      if (!result || result.ok !== true || result.cancelled === true) {
        throw createMediaError(result, "Oracle could not prepare this replay for its internal viewer.");
      }
      if (cleanText(result.cacheKey, 64).toLowerCase() !== cacheKey.toLowerCase()) {
        throw createMediaError({ errorCode: "MEDIA_CACHE_IDENTITY_MISMATCH" }, "Oracle rejected a mismatched replay preview cache result.");
      }
      let thumbnail = null;
