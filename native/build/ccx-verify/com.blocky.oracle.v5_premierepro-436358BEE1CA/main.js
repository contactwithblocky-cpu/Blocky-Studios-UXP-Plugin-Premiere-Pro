...
class ReplayGridView {
  ...
  createCard(replay) {
    ...
    card.setAttribute(
      "aria-label",
      ready
        ? `${replay.title}. Double-click or press Enter to play in Oracle Viewer. Drag to place it on the Premiere timeline.`
...
class OraclePanelController {
  constructor() {
    ...
    if (!replayMediaApi || typeof replayMediaApi.ReplayMediaPipeline !== "function") {
      throw new Error("Oracle replay media module did not load.");
    }
...
async function verifyMinecraftFont(packagedFonts) {
  ...
  if (documentFontsSupported) {
    const loadResults = await Promise.all(families.map(async (family) => {
      try {
        await Promise.race([
          fontSet.load(`16px "${family}"`, "Oracle 0123456789"),
          delay(1500).then(() => { throw new Error("Font loading timed out."); }),
        ]);
        return [family, fontSet.check(`16px "${family}"`, "Oracle 0123456789") === true];
      } catch (error) {
        return [family, false];
      }
    }));
    for (const [family, loaded] of loadResults) documentFontResults.set(family, loaded);
  ...
  for (const family of families) {
    ...
    results.push({
      ...
      documentFontLoaded: documentFontsSupported ? documentFontResults.get(family) === true : null,
    });
  }
  ...
  const documentFacesReady = !documentFontsSupported || families.every((family) => documentFontResults.get(family) === true);
...
class OracleM7PanelHostRegistry {
  ...
  attachMainPanelShell(rootValue) {
    ...
    if (!shell || typeof shell !== "object") {
      throw new Error("Oracle's canonical main-panel shell is unavailable.");
    }
    if (root !== shell && typeof shell.contains === "function" && shell.contains(root)) {
      throw new Error("Premiere supplied a main-panel root inside Oracle's own shell.");
    }
