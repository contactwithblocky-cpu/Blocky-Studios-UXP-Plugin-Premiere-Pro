...
(function exposeOracleReplayViewer(globalScope, factory) {
  ...
  function actionableOpenError(error, fallbackMessage) {
    ...
    if (/unsupported|codec|decoder|decode|media\s+format|invalid\s+format/i.test(signature)) {
      return {
        ...
        message: "Oracle could not prepare a compatible internal preview for this replay. Retry after verifying the source file is readable.",
      };
    }
    return details;
  }
    ...
    async open(replay, context = {}) {
      ...
      if (this.mediaPipeline) {
        ...
        if (this.isDirectlyPlayable(prepared)) {
          notify("direct-source-ready", "Opening original replay in Oracle Viewer…");
        } else {
          notify("preparing-preview", "Preparing an Oracle Viewer preview…");
          ...
          if (!playbackPath || prepared.previewReady !== true) {
            throw Object.assign(new Error("Oracle could not prepare a compatible internal replay preview."), {
              code: "PREVIEW_TRANSCODE_FAILED",
            });
          }
          playbackPhase = "preview-ready";
          managedPreview = true;
          notify("preview-ready", "Opening managed preview in Oracle Viewer…");
        }
      } else {
        notify("direct-source-ready", "Opening original replay in Oracle Viewer…");
      }
      ...
      return new Promise((resolve, reject) => {
        ...
        const onError = () => fail(
          "Oracle Viewer could not decode the prepared replay preview.",
        );
        const onMetadata = () => {
          cleanup();
          if (generation !== this.session) return;
        ...
        timer = setTimeout(() => fail(
          "Oracle Viewer timed out while opening the prepared replay preview.",
  ...
  class ReplayViewerController {
    ...
    sync() {
      ...
      if (this.elements.mode) this.elements.mode.textContent = internal ? "Oracle Viewer" : "Premiere Source Monitor";
      ...
      setDisabled(this.elements.stop, !open || !internal, "Open a replay in Oracle Viewer to stop playback.");
      ...
      const audioReason = !internal
        ...
        : !state.hasAudio
          ...
          : "Open a replay in Oracle Viewer to control audio.";
      ...
      if (this.elements.support) {
        this.elements.support.textContent = internal
          ? (state.managedPreview
            ? "Oracle Viewer is using a local managed preview; the original replay stays unchanged."
            : "Oracle Viewer is playing the original replay directly without changing it.")
          : "Premiere Source Monitor playback is explicit and does not replace Oracle Viewer automatically.";
      }
    ...
    async openReplay(replay) {
      ...
      if (!this.adapterAvailable()) {
        if (this.isOpen()) {
          return this.reportRejectedSwitch(
            { code: internal ? "ORACLE_VIDEO_UNAVAILABLE" : "SOURCE_MONITOR_UNAVAILABLE", message: internal ? "Oracle Viewer video playback is unavailable in this UXP host." : "Premiere Source Monitor viewer controls are unavailable in this host." },
            internal ? "Oracle Viewer playback is unavailable." : "Premiere Source Monitor viewer controls are unavailable.",
          );
        }
      ...
      this.commit({
        ...
        canSeek: false,
        canFrameStep: false,
        status: internal ? "Opening replay in Oracle Viewer…" : "Opening replay in Premiere Source Monitor…",
        error: null,
        hasAudio: false,
        muted: false,
        volume: 1,
        managedPreview: false,
        rateMenuOpen: false,
      }, internal ? `Opening ${normalized.title} in Oracle Viewer.` : `Opening ${normalized.title} in Premiere Source Monitor.`);
      ...
      return this.enqueue(async () => {
        ...
        try {
          ...
          return this.reportError(
            error,
            internal
              ? "Oracle could not open an internal preview for this replay. Retry after verifying the source file is readable."
        ...
        if (![SOURCE_MONITOR_MODE, INTERNAL_VIDEO_MODE].includes(mode) || mode !== requestedMode || !token) {
          ...
          return this.reportError(
            { code: internal ? "INVALID_ORACLE_VIDEO_SESSION" : "INVALID_SOURCE_MONITOR_SESSION", message: internal ? "Oracle Viewer did not return a verifiable media session for this replay." : "Premiere did not return a verifiable Source Monitor session for this replay." },
            internal ? "Oracle Viewer did not return a verifiable media session." : "Premiere did not return a verifiable Source Monitor session.",
            { openFailure: true },
          );
        }
        ...
        this.commit({
          ...
          rateMenuOpen: false,
          status: internal ? "Ready in Oracle Viewer." : "Open in Premiere Source Monitor.",
          error: null,
        }, internal ? `${normalized.title} opened in Oracle Viewer.` : `${normalized.title} opened in Premiere Source Monitor.`);
    ...
    stopPlayback() {
      ...
      return this.enqueue(async () => {
        ...
        this.commit({ playing: false, playbackPhase: "paused", positionSeconds: 0, status: "Stopped in Oracle Viewer.", error: null });
        return true;
      });
    }
    ...
    setPlaying(playing) {
      ...
      if (!next) {
        this.stopPolling();
        this.commit({ playing: false, playbackPhase: "paused", status: internal ? "Pausing in Oracle Viewer…" : "Pausing in Premiere Source Monitor…", error: null });
      }
      return this.enqueue(async () => {
        ...
        try {
          ...
          return this.reportError(error, `${internal ? "Oracle Viewer" : "Premiere"} could not ${next ? "play" : "pause"} this replay.`);
        }
        ...
        this.commit({
          ...
          status: next
            ? (internal ? "Playing in Oracle Viewer." : "Playing in Premiere Source Monitor.")
            : (internal ? "Paused in Oracle Viewer." : "Paused in Premiere Source Monitor."),
          error: null,
    ...
    async readPositionOnce(generation = this.generation, token = this.ownershipToken) {
      ...
      try {
        ...
        if (!this.isCurrent(generation, token)) return false;
        return this.reportError(error, this.state.mode === INTERNAL_VIDEO_MODE ? "Oracle Viewer could not read the replay position." : "Premiere could not read the Source Monitor position.");
      }
    }
          ...
          try {
            ...
            this.commit({
              ...
              status: this.state.playing
                ? (this.state.mode === INTERNAL_VIDEO_MODE ? "Playing in Oracle Viewer." : "Playing in Premiere Source Monitor.")
                : (this.state.mode === INTERNAL_VIDEO_MODE ? "Paused in Oracle Viewer." : "Paused in Premiere Source Monitor."),
              error: null,
            });
            changed = true;
          } catch (error) {
            if (this.isCurrent(pending.generation, pending.token)) {
              this.reportError(error, this.state.mode === INTERNAL_VIDEO_MODE ? "Oracle Viewer could not seek this replay." : "Premiere could not seek this replay.");
            }
          }
        }
        return changed;
