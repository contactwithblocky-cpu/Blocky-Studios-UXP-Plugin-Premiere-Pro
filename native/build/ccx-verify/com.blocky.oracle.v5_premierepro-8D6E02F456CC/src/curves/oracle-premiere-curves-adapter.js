...
(function exposeOraclePremiereCurvesAdapter(globalScope, factory) {
...
})(typeof window !== "undefined" ? window : null, function createOraclePremiereCurvesAdapterApi(globalScope) {
  ...
  async function readTrackDescriptor(api, item) {
    ...
    const [trackIndex, start, end, inPoint, outPoint, duration, speed, reversed, mediaType, projectItem, matchName, name, type] = await Promise.all([
      ...
      typeof item.getInPoint === "function" ? tracePremiereCall("TrackItem.getInPoint", () => item.getInPoint()) : null,
      typeof item.getOutPoint === "function" ? tracePremiereCall("TrackItem.getOutPoint", () => item.getOutPoint()) : null,
      typeof item.getDuration === "function" ? tracePremiereCall("TrackItem.getDuration", () => item.getDuration()) : null,
      typeof item.getSpeed === "function" ? tracePremiereCall("TrackItem.getSpeed", () => item.getSpeed()) : Number.NaN,
      typeof item.isSpeedReversed === "function" ? tracePremiereCall("TrackItem.isSpeedReversed", () => item.isSpeedReversed()) : true,
    ...
    let projectItemId = "";
    try {
      if (projectItem && typeof projectItem.getId === "function") projectItemId = cleanText(tracePremiereCall("ProjectItem.getId", () => projectItem.getId()), 256);
      if (!projectItemId && api && api.UniqueSerializeable && typeof api.UniqueSerializeable.cast === "function") {
        ...
        if (serializable && typeof serializable.getUniqueID === "function") {
          projectItemId = guidText(tracePremiereCall("UniqueSerializeable.getUniqueID", () => serializable.getUniqueID()));
        }
      }
    } catch (error) {
      projectItemId = "";
    ...
    return {
      item,
      chain: await tracePremiereCall("TrackItem.getComponentChain", () => item.getComponentChain()),
  ...
  async function readEndpoint(param, timeRecord) {
    ...
    const keyframe = tracePremiereCall("ComponentParam.getKeyframePtr", () => param.getKeyframePtr(timeRecord.raw));
    ...
    if (typeof keyframe.getTemporalInterpolationMode === "function") {
      interpolationMode = await tracePremiereCall("Keyframe.getTemporalInterpolationMode", () => keyframe.getTemporalInterpolationMode());
  ...
  async function inspectTrackItem(api, context, item, nativeModes, bakedRuntimeProof, limits, detectionSource = "selection") {
    ...
    const componentCount = Number(tracePremiereCall("ComponentChain.getComponentCount", () => chain.getComponentCount()));
    ...
    for (let componentIndex = 0; componentIndex < componentCount; componentIndex += 1) {
      try {
        const component = tracePremiereCall("ComponentChain.getComponentAtIndex", () => chain.getComponentAtIndex(componentIndex), { componentIndex });
        ...
        const [rawMatchName, rawDisplayName] = await Promise.all([
          ...
          typeof component.getDisplayName === "function" ? tracePremiereCall("Component.getDisplayName", () => component.getDisplayName()) : "",
        ]);
        ...
        const paramCount = Number(tracePremiereCall("Component.getParamCount", () => component.getParamCount()));
        ...
        for (let paramIndex = 0; paramIndex < paramCount; paramIndex += 1) {
          try {
            const param = tracePremiereCall("Component.getParam", () => component.getParam(paramIndex), { paramIndex });
            ...
            if (await tracePremiereCall("ComponentParam.areKeyframesSupported", () => param.areKeyframesSupported()) !== true) continue;
            const rawTimes = tracePremiereCall("ComponentParam.getKeyframeListAsTickTimes", () => param.getKeyframeListAsTickTimes());
            ...
            if (inferredKind === "unknown" && typeof param.getValueAtTime === "function" && context.playhead) {
              try { inferredKind = valueKind(await tracePremiereCall("ComponentParam.getValueAtTime", () => param.getValueAtTime(context.playhead))); } catch (error) { /* display-only inference */ }
            }
            const identity = {
  ...
  async function clipsAtPlayhead(api, sequence, playheadTicks, limits) {
    ...
    const rawTrackCount = Number(await tracePremiereCall("Sequence.getVideoTrackCount", () => sequence.getVideoTrackCount()));
    ...
    for (let trackIndex = rawTrackCount - 1; trackIndex >= firstTrack; trackIndex -= 1) {
      let track;
      try { track = await tracePremiereCall("Sequence.getVideoTrack", () => sequence.getVideoTrack(trackIndex), { trackIndex }); } catch (error) { continue; }
      if (!track || typeof track.getTrackItems !== "function") continue;
      let rawItems;
      try { rawItems = await tracePremiereCall("Track.getTrackItems", () => track.getTrackItems(clipType, false), { trackIndex }); } catch (error) { continue; }
      let items = [];
  ...
  class PremiereCurvesAdapter {
    ...
    bindSequenceEvents(sequence) {
      ...
      for (const eventName of eventNames) {
        try {
          tracePremiereCall("EventManager.addEventListener", () => manager.addEventListener(sequence, eventName, this.hostEventListener), { eventName });
    ...
    unbindSequenceEvents() {
      ...
      if (manager && typeof manager.removeEventListener === "function") {
        for (const entry of this.observedSequenceListeners.splice(0)) {
          try { tracePremiereCall("EventManager.removeEventListener", () => manager.removeEventListener(entry.sequence, entry.eventName, this.hostEventListener), { eventName: entry.eventName }); } catch (error) { /* cleanup is best effort */ }
        }
      } else {
    ...
    async inspectHost() {
      ...
      try {
        const settings = typeof sequence.getSettings === "function" ? await tracePremiereCall("Sequence.getSettings", () => sequence.getSettings()) : null;
        const hostFrameRate = settings && typeof settings.getVideoFrameRate === "function"
          ? tracePremiereCall("SequenceSettings.getVideoFrameRate", () => settings.getVideoFrameRate())
          : null;
      ...
      if (typeof sequence.getSelection === "function") {
        try {
          ...
          if (selection && typeof selection.getTrackItems === "function") {
            const rawSelectedItems = await tracePremiereCall("Selection.getTrackItems", () => selection.getTrackItems());
            ...
            try {
              ...
              transactionResult = tracePremiereCall("Project.executeTransaction", () => project.executeTransaction((compoundAction) => {
                ...
                for (const entry of actionable) {
                  let action;
                  try {
                    action = tracePremiereCall("ComponentParam.createSetInterpolationAtKeyframeAction", () => entry.hostBinding.param.createSetInterpolationAtKeyframeAction(
                      ...
                      true,
                    ), { bindingId: entry.operation.bindingId });
                  } catch (error) {
    ...
    async reconcilePriorBakeReceiptsForPlan(inspection) {
      ...
      for (const [ownershipToken, ownershipRecord] of Array.from(this.issuedOwnershipReceipts.entries())) {
        ...
        for (const operation of receipt.operations) {
          ...
          const currentTicks = new Set(tracePremiereCall("ComponentParam.getKeyframeListAsTickTimes", () => hostBinding.param.getKeyframeListAsTickTimes())
          ...
          for (const ownedKey of operation.createdKeys) {
            ...
            const hostTime = tracePremiereCall("TickTime.createWithTicks", () => this.api.TickTime.createWithTicks(ownedKey.ticks));
            let current;
          ...
          try {
            ...
            transactionResult = tracePremiereCall("Project.executeTransaction", () => project.executeTransaction((compoundAction) => {
              ...
              for (const createAction of actionFactories) {
                ...
                if (tracePremiereCall("CompoundAction.addAction", () => compoundAction.addAction(action)) !== true) throw new CurvesAdapterError(REASON.ACTION_REJECTED);
              }
            }, undoString));
          } catch (error) {
            callbackError = error;
      ...
      for (const operation of plan.operations) {
        ...
        for (const candidate of operation.candidates) {
          actionFactories.push(() => {
            const hostTime = tracePremiereCall("TickTime.createWithTicks", () => this.api.TickTime.createWithTicks(candidate.ticks));
            const keyframe = tracePremiereCall("ComponentParam.createKeyframe", () => resolved.hostBinding.param.createKeyframe(candidate.value));
            if (!keyframe || typeof keyframe !== "object") throw new CurvesAdapterError(REASON.ACTION_CREATION_FAILED);
            keyframe.position = hostTime;
            return tracePremiereCall("ComponentParam.createAddKeyframeAction", () => resolved.hostBinding.param.createAddKeyframeAction(keyframe));
          });
        }
      }
    ...
    async cleanupOwnedKeys(receipt, options = {}) {
      ...
      for (const entry of resolvedOperations) {
        for (const ownedKey of entry.operation.createdKeys) {
          actionFactories.push(() => tracePremiereCall("ComponentParam.createRemoveKeyframeAction", () => entry.hostBinding.param.createRemoveKeyframeAction(
            tracePremiereCall("TickTime.createWithTicks", () => this.api.TickTime.createWithTicks(ownedKey.ticks)),
            true,
          )));
        }
      }
      ...
      for (const entry of resolvedOperations) {
        const remaining = new Set(tracePremiereCall("ComponentParam.getKeyframeListAsTickTimes", () => entry.hostBinding.param.getKeyframeListAsTickTimes())
          .map((time) => normalizeTickString(time)).filter(Boolean));
