...
(function exposeOracleReplayMedia(globalScope, factory) {
  ...
  const api = factory(library, globalScope);
  if (typeof module === "object" && module && module.exports) module.exports = api;
  if (globalScope) Reflect.set(globalScope, "OracleReplayMedia", api);
})(typeof window !== "undefined" ? window : null, function createOracleReplayMediaApi(library, globalScope) {
  ...
  function traceNativeCall(name, operation, details = {}) {
    ...
    return telemetry && typeof telemetry.invoke === "function"
      ? telemetry.invoke("native-addon", name, operation, details)
      : operation();
  }
  ...
  class ReplayMediaPipeline {
    ...
    async run(record, cacheKey, variant, includePreview, generation) {
      ...
      let pending;
      try {
        pending = traceNativeCall("prepareReplayMedia", () => this.nativeAddon.prepareReplayMedia({
    ...
    cancelRecord(record) {
      ...
      for (const request of Array.from(this.activeRequests.values())) {
        ...
        try {
          const result = traceNativeCall("cancelReplayMedia", () => this.nativeAddon.cancelReplayMedia(request.requestId), { requestId: request.requestId });
    ...
    cancelAll() {
      for (const request of Array.from(this.activeRequests.values())) {
        try { traceNativeCall("cancelReplayMedia", () => this.nativeAddon.cancelReplayMedia(request.requestId), { requestId: request.requestId }); } catch (error) { /* Best-effort cancellation. */ }
      }
