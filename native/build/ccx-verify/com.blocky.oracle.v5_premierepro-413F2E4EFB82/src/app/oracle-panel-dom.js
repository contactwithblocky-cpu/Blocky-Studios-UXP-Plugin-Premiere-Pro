...
(function exposeOraclePanelDom(globalScope, factory) {
  ...
  function assertNoDuplicateIds(root, label = "Blocky Studios panel DOM") {
  ...
  function capturePristineBlueprint(documentRef, sources, options = {}) {
    ...
    const snapshots = sourceList.map((source, index) => {
      ...
      if (!node || typeof node.cloneNode !== "function") {
        throw new Error(`Blocky Studios panel blueprint source ${index + 1} is unavailable.`);
      }
    ...
    if (duplicates.length) {
      const summary = duplicates.map((entry) => `${entry.id} (${entry.count})`).join(", ");
      throw new Error(`Blocky Studios panel blueprint contains duplicate IDs: ${summary}`);
    }
    const originalIds = [];
  ...
  function createRootScopedElementMap(root, options = {}) {
    ...
    for (const element of elementsWithin(resolvedRoot)) {
      ...
      if (strict && output[key] && output[key] !== element) {
        throw new Error(`Blocky Studios panel root contains duplicate element key: ${key}`);
      }
      output[key] = element;
    }
    return output;
  ...
  function instantiatePanelBlueprint(blueprint, options = {}) {
    ...
    if (!captured) throw new TypeError("instantiatePanelBlueprint requires a Blocky Studios panel blueprint.");
    ...
    if (duplicates.length) throw new Error("Blocky Studios panel blueprint clone contains duplicate IDs.");
  ...
  function createDedicatedPanelShell(documentRef, options = {}) {
    ...
    appendElement(documentRef, card, "span", "oracle-kicker", cleanText(options.kicker, 120) || "Blocky Studios workspace");
    ...
    const message = appendElement(
      ...
      "p",
      "",
      cleanText(options.message, 500) || "Connecting the Blocky Studios workspace.",
    );
  ...
  function setDedicatedPanelShellState(shell, state, error = null) {
    if (!shell || !shell.root || !shell.state || !shell.title || !shell.message || !shell.content) {
      throw new TypeError("A valid Blocky Studios dedicated panel shell is required.");
    }
    ...
    shell.title.textContent = next === "error" ? "Blocky Studios could not start" : "Loading cinematic tools";
    if (next === "error") {
      const detail = cleanText(error && error.message ? error.message : error, 500);
      shell.message.textContent = detail
        ? `${detail} Reload Blocky Studios in UDT after checking the first UXP Console exception.`
        : "The workspace bootstrap failed. Reload Blocky Studios in UDT after checking the first UXP Console exception.";
    } else {
      shell.message.textContent = "Connecting the Blocky Studios workspace.";
    }
    return next;
  }
