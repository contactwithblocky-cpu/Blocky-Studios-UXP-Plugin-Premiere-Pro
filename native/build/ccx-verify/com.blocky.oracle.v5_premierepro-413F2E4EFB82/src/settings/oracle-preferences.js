...
(function exposeOraclePreferences(globalScope, factory) {
...
})(typeof window !== "undefined" ? window : null, function createOraclePreferencesApi(globalScope) {
  ...
  function normalizeDesignColor(value, previousDefault, currentDefault) {
    const normalized = normalizeHexColor(value, currentDefault);
    return normalized === normalizeHexColor(previousDefault, "") ? currentDefault : normalized;
  }
  ...
  function contrastReportForMode(normalized, mode) {
    ...
    // Secondary copy and focus rings are rendered on persistent owning
    // surfaces. Hover promotes its copy to primary text and never owns a
    // standalone focus ring, so folding hover into these minima rejects the
    // exact Blocky Studios reference palette for a role it never serves.
    const persistentRoles = ["background", "header", "surface", "raised"];
    const persistentMinimum = (values) => Math.min(...persistentRoles.map((role) => values[role]));
    return {
      mode,
      palette,
      background,
      ...
      mutedMinimum: persistentMinimum(mutedRatios),
      focusMinimum: persistentMinimum(focusRatios),
      allSurfaceMutedMinimum: minimum(mutedRatios),
      allSurfaceFocusMinimum: minimum(focusRatios),
  ...
  function normalizePreferences(input, legacy = {}) {
    ...
    return {
      ...
      appearance: {
        ...
        border: normalizeDesignColor(appearance.border, "#343A44", defaults.appearance.border),
        lightBorder: normalizeDesignColor(appearance.lightBorder, "#CBC7BD", defaults.appearance.lightBorder),
        focus: normalizeDesignColor(appearance.focus, "#FFFFFF", defaults.appearance.focus),
        lightFocus: normalizeDesignColor(appearance.lightFocus, "#111318", defaults.appearance.lightFocus),
        fontScale: clamp(
      ...
      curves: {
        ...
        gridColor: normalizeDesignColor(curves.gridColor, "#3C3C3C", defaults.curves.gridColor),
        curveColor: normalizeDesignColor(curves.curveColor, "#1D78C8", defaults.curves.curveColor),
  ...
  function releaseDocumentThemeBinding(doc) {
    const binding = doc && DOCUMENT_THEME_BINDINGS.get(doc);
    if (!binding) return false;
    if (binding.mediaQuery && binding.listener) {
      if (typeof binding.mediaQuery.removeEventListener === "function") {
        binding.mediaQuery.removeEventListener("change", binding.listener);
      } else if (typeof binding.mediaQuery.removeListener === "function") {
        binding.mediaQuery.removeListener(binding.listener);
      }
    }
    binding.listener = null;
    binding.mediaQuery = null;
    binding.values = null;
    DOCUMENT_THEME_BINDINGS.delete(doc);
    return true;
  }
  ...
  class OraclePreferencesController {
    constructor(elements, options = {}) {
      ...
      this.ownsDocumentThemeBinding = options.ownsDocumentThemeBinding !== false;
    ...
    open(section = "profile") {
      ...
      this.syncAllControls();
      // Premiere's docked UXP compositor may not deliver an animation frame
      // after unhiding a modal surface. Commit the usable state synchronously
      // so Preferences cannot remain transparent and pointer-inert.
      ...
      if (telemetry && typeof telemetry.interaction === "function") {
        const root = this.elements.preferencesPanel.closest && this.elements.preferencesPanel.closest("[data-oracle-panel-root], .oracle-panel");
        telemetry.interaction({ root, panelId: root && root.dataset && root.dataset.oraclePanelRoot || "oraclePanel", name: "preferences", action: "open", state: this.activeSection });
      }
    }
    ...
    close(restoreFocus = true) {
      ...
      if (restoreFocus) {
        ...
        if (target) Promise.resolve().then(() => {
          if (!this.isOpen && target.isConnected !== false && typeof target.focus === "function") target.focus();
        });
      }
      const telemetry = globalScope && Reflect.get(globalScope, "oraclePlatformTelemetry");
      if (telemetry && typeof telemetry.interaction === "function") {
        const root = this.elements.preferencesPanel.closest && this.elements.preferencesPanel.closest("[data-oracle-panel-root], .oracle-panel");
        telemetry.interaction({ root, panelId: root && root.dataset && root.dataset.oraclePanelRoot || "oraclePanel", name: "preferences", action: "close", state: this.activeSection });
      }
      this.restoreFocus = null;
    }
    ...
    showSection(section) {
      ...
      const telemetry = globalScope && Reflect.get(globalScope, "oraclePlatformTelemetry");
      if (previousSection !== section && telemetry && typeof telemetry.tabSwitch === "function") {
        telemetry.tabSwitch({
          root: this.elements.preferencesPanel.closest && this.elements.preferencesPanel.closest("[data-oracle-panel-root], .oracle-panel"),
          panelId: "oraclePanel",
          group: "preferences",
          from: previousSection,
          to: section,
          trigger: "interaction-or-controller",
        });
      }
    }
    ...
    syncRootList() {
      const renderList = (selector, roots, actionPrefix, emptyMessage, labelPrefix) => {
        ...
        remove.type = "button";
        remove.className = "oracle-button oracle-button--danger";
    ...
    destroy() {
      ...
      if (this.document) {
        ...
        if (this.ownsDocumentThemeBinding) releaseDocumentThemeBinding(this.document);
      }
    }
  }
