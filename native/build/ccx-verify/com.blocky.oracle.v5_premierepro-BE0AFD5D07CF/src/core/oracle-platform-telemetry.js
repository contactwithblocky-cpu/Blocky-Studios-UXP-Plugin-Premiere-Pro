...
(function exposeOraclePlatformTelemetry(globalScope, factory) {
  ...
  function createOraclePlatformTelemetry(options = {}) {
    ...
    function tabSwitch(details = {}) {
      ...
      if (root && uiRuntime && typeof uiRuntime.capture === "function") {
        try { uiRuntime.capture(root, `tab:${payload.group}:${payload.to}`); } catch (error) { /* Tab behavior remains authoritative. */ }
      }
      return entry;
    }
    ...
    return Object.freeze({ invoke, tabSwitch, claimResource, audit });
  }
