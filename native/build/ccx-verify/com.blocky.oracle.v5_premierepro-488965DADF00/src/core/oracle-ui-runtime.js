...
(function exposeOracleUiRuntime(globalScope, factory) {
  ...
  if (!Reflect.get(globalScope, "oracleButtonPaintBridge")) {
    Reflect.set(globalScope, "oracleButtonPaintBridge", api.installButtonPaintBridge({
      window: globalScope,
      document: Reflect.get(globalScope, "document"),
    }));
  }
  ...
  const BUTTON_PAINT_SELECTOR = "button:not(.oracle-layer-backdrop)";

  function installButtonPaintBridge(options = {}) {
    ...
    const existing = runtimeWindow && Reflect.get(runtimeWindow, "oracleButtonPaintBridge");
    if (existing && typeof existing.snapshot === "function") return existing;

    const trackedButtons = new Set();
    const MutationObserverCtor = runtimeWindow && runtimeWindow.MutationObserver ||
      typeof MutationObserver === "function" && MutationObserver || null;
    let observer = null;
    let destroyed = false;

    function isPaintNode(node) {
      return Boolean(node && node.nodeType === 1 && node.classList && node.classList.contains("oracle-button-paint"));
    }

    function removeMirroredIdentity(node) {
      ...
      if (typeof node.querySelectorAll === "function") {
        for (const descendant of node.querySelectorAll("[id], [name], [aria-label], [aria-labelledby], [aria-describedby], [role], [tabindex]")) {
          removeMirroredIdentity(descendant);
        }
      }
    }

    function mirrorButton(button) {
      if (destroyed || !button || button.nodeType !== 1 || String(button.tagName || "").toLowerCase() !== "button") return false;
      if (button.classList && button.classList.contains("oracle-layer-backdrop")) return false;
      // Read the authored ink before enabling the paint-ready rule. That rule
      // makes only the semantic source label transparent so Premiere cannot
      // composite it a second time above the host-button paint workaround.
      if (typeof button.removeAttribute === "function") button.removeAttribute("data-oracle-paint-ready");
      let authoredColor = "";
      try {
        const computed = runtimeWindow && typeof runtimeWindow.getComputedStyle === "function"
          ? runtimeWindow.getComputedStyle(button)
          : null;
        authoredColor = String(computed && computed.color || "").trim();
      } catch (error) {
        authoredColor = "";
      }
      const existingPaint = Array.from(button.children || []).find(isPaintNode) || null;
      const sourceNodes = Array.from(button.childNodes || []).filter((node) => !isPaintNode(node));
      const paint = runtimeDocument && typeof runtimeDocument.createElement === "function"
        ? runtimeDocument.createElement("span")
        : null;
      if (!paint) return false;
      paint.className = "oracle-button-paint";
      paint.setAttribute("aria-hidden", "true");
      paint.setAttribute("role", "presentation");
      if (authoredColor && paint.style) paint.style.color = authoredColor;
      for (const sourceNode of sourceNodes) {
        if (!sourceNode || typeof sourceNode.cloneNode !== "function") continue;
        const clone = sourceNode.cloneNode(true);
        removeMirroredIdentity(clone);
        paint.appendChild(clone);
      }
      if (existingPaint && existingPaint.parentNode === button) button.removeChild(existingPaint);
      if (typeof button.insertBefore === "function") button.insertBefore(paint, button.firstChild || null);
      else if (typeof button.appendChild === "function") button.appendChild(paint);
      if (typeof button.setAttribute === "function") button.setAttribute("data-oracle-paint-ready", "true");
      trackedButtons.add(button);
      return true;
    }

    function scanNode(node) {
      if (destroyed || !node || node.nodeType !== 1) return;
      if (typeof node.matches === "function" && node.matches(BUTTON_PAINT_SELECTOR)) mirrorButton(node);
      if (typeof node.querySelectorAll === "function") {
        for (const button of node.querySelectorAll(BUTTON_PAINT_SELECTOR)) mirrorButton(button);
      }
    }

    function buttonForMutationTarget(target) {
      if (!target) return null;
      const element = target.nodeType === 1 ? target : target.parentElement;
      if (!element || typeof element.closest !== "function") return null;
      if (element.closest(".oracle-button-paint")) return null;
      return element.closest(BUTTON_PAINT_SELECTOR);
    }

    function mutationNeedsMirror(record) {
      ...
      return changed.some((node) => !isPaintNode(node));
    }

    if (runtimeDocument && runtimeDocument.documentElement) {
      scanNode(runtimeDocument.documentElement);
      if (MutationObserverCtor) {
        observer = new MutationObserverCtor((records) => {
          const dirtyButtons = new Set();
          for (const record of records || []) {
            for (const addedNode of Array.from(record && record.addedNodes || [])) {
              if (!isPaintNode(addedNode)) scanNode(addedNode);
            }
            if (!mutationNeedsMirror(record)) continue;
            const button = buttonForMutationTarget(record.target);
            if (button) dirtyButtons.add(button);
          }
          for (const button of dirtyButtons) mirrorButton(button);
          for (const button of Array.from(trackedButtons)) {
        ...
        observer.observe(runtimeDocument.documentElement, {
          ...
          characterData: true,
          attributes: true,
          attributeFilter: ["src", "alt", "class", "hidden"],
        });
      }
    }
    ...
    const handle = Object.freeze({
      rescan() {
        if (runtimeDocument && runtimeDocument.documentElement) scanNode(runtimeDocument.documentElement);
        return this.snapshot();
      },
      snapshot() {
        return Object.freeze({
          available: Boolean(runtimeDocument && runtimeDocument.documentElement),
          observerCount: observer ? 1 : 0,
          trackedButtonCount: trackedButtons.size,
          mirrorCount: trackedButtons.size,
          destroyed,
        });
      },
      destroy() {
        ...
        for (const button of trackedButtons) {
          if (!button) continue;
          for (const paint of Array.from(button.children || []).filter(isPaintNode)) paint.remove();
          if (typeof button.removeAttribute === "function") button.removeAttribute("data-oracle-paint-ready");
        }
        trackedButtons.clear();
        if (runtimeWindow && Reflect.get(runtimeWindow, "oracleButtonPaintBridge") === handle) {
          Reflect.deleteProperty(runtimeWindow, "oracleButtonPaintBridge");
        }
        return true;
      },
    });
  ...
  function rootDimensions(root) {
    ...
    // Premiere's UXP DOM can paint a nested flex item while reporting a zero
    // bounding rectangle and zero client box for that same node. The offset
    // box is the host's remaining synchronous layout signal in that case. Use
    // the largest finite signal so a transient zero from one API cannot turn a
    // visibly rendered workspace/control into a false zero-size diagnostic.
    return {
      width: Math.max(
        finiteDimension(rectangle && rectangle.width),
        finiteDimension(root && root.clientWidth),
        finiteDimension(root && root.offsetWidth),
      ),
      height: Math.max(
        finiteDimension(rectangle && rectangle.height),
        finiteDimension(root && root.clientHeight),
        finiteDimension(root && root.offsetHeight),
      ),
    };
  }
  ...
  function visibleControlInventory(root, getComputedStyleFunction, customElementsRegistry) {
    ...
    for (const element of Array.from(root.querySelectorAll(selector))) {
      ...
      const computed = Object.freeze({
        display: cleanHealthText(style && style.display, 48),
        position: cleanHealthText(style && style.position, 48),
        overflowX: cleanHealthText(style && style.overflowX, 48),
        overflowY: cleanHealthText(style && style.overflowY, 48),
        flexDirection: cleanHealthText(style && style.flexDirection, 48),
        flexWrap: cleanHealthText(style && style.flexWrap, 48),
        alignItems: cleanHealthText(style && style.alignItems, 48),
        justifyContent: cleanHealthText(style && style.justifyContent, 48),
        color: cleanHealthText(style && style.color, 80),
        backgroundColor: cleanHealthText(style && style.backgroundColor, 80),
      });
  ...
  function createOracleUiRuntime(options = {}) {
    ...
    function measure(record, forceNotify = false) {
      ...
      notify(record);
      const becameMeasurable = record.snapshot.width > 0 && record.snapshot.height > 0 &&
        (!previous || previous.width <= 0 || previous.height <= 0);
      if (record.proofLogged && becameMeasurable && !record.measurableProofLogged) {
        record.measurableProofLogged = true;
        logRenderProof(record, "first-measurable-layout");
      }
    ...
    function setRecordVisible(record, value) {
      ...
      if (record.visibilityOverride === next) {
        // Premiere can create a dock tab while its root is still zero-sized and
        // later issue another show for the same logical visible state. Treat
        // that idempotent show as a layout synchronization point without
        // creating another observer, listener, timer, or render loop.
        if (next === true) {
          measure(record, true);
          scheduleProof(record, "panel-visible-settled");
        }
        return false;
      }
    ...
    function auditRecord(record) {
      ...
      const localAssets = localAssetReport(
        root,
    ...
    function logRenderProof(record, reason = "manual") {
      ...
      const proof = Object.freeze({
        ...
        localAssets: report.localAssets,
      });
      ...
      recordRuntimeDiagnostic("info", "PLATFORM_RENDER_PROOF", {
        ...
        localAssetFailureCount: report.localAssets.failures.length,
      });
      return proof;
    }
    ...
    function mount(root, mountOptions = {}) {
      ...
      record.measurableProofLogged = Boolean(record.snapshot && record.snapshot.width > 0 && record.snapshot.height > 0);
    ...
    function audit(auditOptions = {}) {
      ...
      const buttonPaintBridge = runtimeWindow && runtimeWindow.oracleButtonPaintBridge &&
        typeof runtimeWindow.oracleButtonPaintBridge.snapshot === "function"
        ? runtimeWindow.oracleButtonPaintBridge.snapshot()
        : Object.freeze({
            available: false,
            observerCount: 0,
      ...
      return Object.freeze({
        ...
        runtimeOwnership: Object.freeze({
          ...
          buttonPaintObserverCount: buttonPaintBridge.observerCount,
