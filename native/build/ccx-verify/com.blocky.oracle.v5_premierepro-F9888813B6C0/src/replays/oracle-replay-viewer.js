...
(function exposeOracleReplayViewer(globalScope, factory) {
...
})(typeof window !== "undefined" ? window : null, function createOracleReplayViewerApi(globalScope) {
  ...
  function setTransportIcon(element, icon) {
    if (!element) return;
    if (element.dataset) element.dataset.icon = icon;
    if (typeof element.querySelector !== "function") return;
    const image = element.querySelector(".replay-viewer__transport-image");
    const source = TRANSPORT_ICON_SOURCES[icon];
    if (image && source && typeof image.setAttribute === "function") {
  ...
  class ReplayViewerController {
    ...
    sync() {
      ...
      if (this.elements.playPause) {
        setTransportIcon(this.elements.playPause, state.playing ? "pause" : "play");
      ...
      setTransportIcon(this.elements.mute, state.muted ? "muted" : "volume");
    ...
    async openReplay(replay) {
      ...
      if (telemetry && typeof telemetry.interaction === "function") {
        const root = this.elements.root && this.elements.root.closest
          ...
          : this.elements.root;
        telemetry.interaction({ root, panelId: root && root.dataset && root.dataset.oraclePanelRoot || "oraclePanel", name: "replay-viewer", action: "open" });
      }
    ...
    close(reason = "user") {
      ...
      if (telemetry && typeof telemetry.interaction === "function") {
        const root = this.elements.root && this.elements.root.closest
          ? this.elements.root.closest("[data-oracle-panel-root], .oracle-panel") || this.elements.root
          : this.elements.root;
        telemetry.interaction({ root, panelId: root && root.dataset && root.dataset.oraclePanelRoot || "oraclePanel", name: "replay-viewer", action: "close", state: reason });
      }
      const generation = ++this.generation;
      const token = this.ownershipToken;
      this.requestedReplay = null;
