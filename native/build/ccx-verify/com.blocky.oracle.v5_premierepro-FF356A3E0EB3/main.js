...
async function loadNativeDragAddon(loadState = null) {
  ...
  const abandonedResult = () => loadState && loadState.cancelled === true
    ? {
        addon: null,
        diagnostic: nativeDragError(
          "ADDON_LOAD_ABANDONED",
          "Native addon loading completed after the bounded UI bootstrap window.",
          Object.assign({}, runtimeInfo, { bootstrapAbandoned: true }),
        ),
      }
    : null;
  const abandonedAfterRequire = abandonedResult();
  if (abandonedAfterRequire) return abandonedAfterRequire;
  ...
  const abandonedAfterRegistration = abandonedResult();
  if (abandonedAfterRegistration) {
    try { addon.unregisterPackagedFonts(); } catch (error) { /* Best-effort cleanup after bootstrap timeout. */ }
    return abandonedAfterRegistration;
  }
  if (
    !packagedFonts ||
...
async function loadNativeDragAddonForBootstrap(timeoutMs = NATIVE_ADDON_BOOTSTRAP_TIMEOUT_MS) {
  ...
  try {
    return await Promise.race([loadNativeDragAddon(loadState), timeoutResult]);
  } finally {
...
const nativeDragAddonLoadPromise = loadNativeDragAddonForBootstrap().then((loadResult) => {
