...
(function exposeOracleCurvesWorkspace(globalScope, factory) {
...
})(typeof window !== "undefined" ? window : null, function createOracleCurvesWorkspaceApi(defaultCurveMath, defaultPresetApi, globalScope) {
  ...
  class CurvesWorkspaceController {
    ...
    setVisible(visible) {
      this.setLifecycle({ visible, active: this.active });
    }

    setActive(active) {
      this.setLifecycle({ visible: this.visible, active });
    }
    ...
    setLifecycle(value = {}) {
      ...
      if (!nextVisible || !nextActive) {
        this.releaseActivePointerCaptures();
        this.cancelPreviewFrame();
        if (!nextVisible) this.setSettingsOpen(false);
      }
      this.reconcileActivation();
      return true;
    }
    ...
    renderDetectedProperties() {
      ...
      for (const group of this.detectedPropertyGroups.values()) {
        ...
        button.type = "button";
        button.className = "oracle-button oracle-button--segment curves-detected-property";
    ...
    renderOverlays() {
      ...
      if (!group) return;
      const overlays = this.activeBindings.slice(1).map((binding) => ({
        path: curvePathData(binding.controlPoints),
        className: binding.compatible ? "curves-graph__overlay" : "curves-graph__overlay curves-graph__overlay--unsupported",
      }));
      ...
      for (const overlay of overlays) {
        ...
        path.setAttribute("class", overlay.className);
    ...
    onGraphPointerDown(event) {
      ...
      if (telemetry && typeof telemetry.interaction === "function") {
        const root = this.elements.root && this.elements.root.closest
          ...
          : this.elements.root;
        telemetry.interaction({ root, panelId: root && root.dataset && root.dataset.oraclePanelRoot || "oracleCurvesPanel", name: "curves-graph", action: "drag-start", state: `handle-${handleIndex + 1}` });
      }
    }
    ...
    renderPresetList() {
      ...
      for (const entry of entries) {
        ...
        // UXP's native button renderer flattens nested preset metadata. Keep the
        // listbox semantics and keyboard behavior on an ordinary DOM option so
        // preview, name, source and favorite remain independently composable.
        const card = doc.createElement("div");
        card.className = "curves-preset-card";
        card.dataset.curvePresetId = preset.id;
        card.dataset.curvePresetKind = kind;
        card.dataset.curvePresetIndex = String(presets.indexOf(preset));
        card.setAttribute("role", "option");
        card.setAttribute("aria-selected", preset.id === this.selectedPresetId ? "true" : "false");
        card.tabIndex = preset.id === this.presetFocusId ? 0 : -1;
        if (preset.id === this.selectedPresetId) card.classList.add("is-selected");
