...
(function exposeOracleQuickApplyDomain(globalScope, factory) {
  ...
  const RESULT_GROUPINGS = Object.freeze(["type", "category", "none"]);
  ...
  function normalizeSelectionSummary(input = {}) {
    if (Array.isArray(input)) {
      ...
      return {
        ...
        message: input.length
          ...
          : "Select one or more timeline clips to apply an effect or Blocky Studios Recipe.",
      };
    }
    ...
    if (!message) {
      if (totalCount === 0) message = "Select one or more timeline clips to apply an effect or Blocky Studios Recipe.";
      else {
        const parts = [];
        if (videoCount) parts.push(`${videoCount} video`);
        if (audioCount) parts.push(`${audioCount} audio`);
  ...
  function normalizeRecipe(value = {}, index = 0, options = {}) {
    ...
    return {
      id,
      name: cleanText(value.name, 256) || `Blocky Studios Recipe ${index + 1}`,
  ...
  function exportRecipeLibrary(input, options = {}) {
    const sourceRecipes = recipeLibrarySourceRecipes(input);
    if (sourceRecipes.length > MAX_RECIPES) throw new TypeError(`Blocky Studios Recipe export exceeds the ${MAX_RECIPES}-recipe limit.`);
  ...
  function importRecipeLibrary(serialized, options = {}) {
    ...
    if (filename.endsWith(".prfpset")) throw new TypeError("Premiere .prfpset files are proprietary and are not Blocky Studios Recipe files.");
    const text = String(serialized == null ? "" : serialized);
    if (!text || utf8ByteLength(text) > MAX_IMPORT_BYTES) throw new TypeError("Blocky Studios Recipe import is empty or exceeds the 2 MB limit.");
    let parsed;
    try { parsed = JSON.parse(text); } catch (error) { throw new TypeError("Blocky Studios Recipe JSON is invalid."); }
    if (!isPlainObject(parsed) || cleanText(parsed.schema, 128) !== RECIPE_LIBRARY_SCHEMA) throw new TypeError("This file is not a Blocky Studios Recipe library.");
    if (finiteInteger(parsed.version, 0, 0, 1000) > RECIPE_LIBRARY_VERSION) throw new TypeError("This Blocky Studios Recipe library was created by a newer version of Blocky Studios.");
    const importedRecipes = recipeLibrarySourceRecipes(parsed);
    if (importedRecipes.length > MAX_RECIPES) {
      throw new TypeError(`Blocky Studios Recipe import exceeds the ${MAX_RECIPES}-recipe limit.`);
    }
    ...
    if (!library.recipes.length && importedRecipes.length) throw new TypeError("The Blocky Studios Recipe library contains no valid recipes.");
    return library;
  }
  ...
  function recipeCompatibility(recipe, selection, explicitValue) {
    ...
    if (!selection.totalCount) return { compatible: false, reason: "Select at least one timeline clip before applying this Blocky Studios Recipe.", compatibleCount: 0, skippedCount: 0 };
  ...
  function persistenceFailure(error, message) {
    return {
      ...
      message: cleanText(error && error.message, 2048) || message || "Blocky Studios could not save this Quick Apply change.",
      persistence: { ok: false, code: "PERSISTENCE_FAILED" },
    };
  }

  function normalizeResultGrouping(value, fallback = "type") {
    const grouping = cleanText(value, 32).toLocaleLowerCase("en-US");
    return RESULT_GROUPINGS.includes(grouping) ? grouping : fallback;
  }

  function resultGroupDescriptor(target, grouping) {
    if (grouping === "none") return null;
    if (target && target.kind === "recipe") {
      return { id: "recipes", label: "Blocky Studios Recipes", rank: 10000 };
    }
    if (grouping === "type") {
      const audio = target && target.mediaType === "audio";
      return {
        id: audio ? "audio" : "video",
        label: audio ? "Audio effects" : "Video effects",
        rank: audio ? 1 : 0,
      };
    }
    const category = target && target.categoryAvailable ? cleanText(target.category, 256) : "";
    if (!category) {
      return {
        id: "category-unavailable",
        label: "Category unavailable from Premiere",
        rank: 9000,
      };
    }
    return {
      id: `category-${hash32(category.toLocaleLowerCase("en-US"))}`,
      label: category,
      rank: 100,
    };
  }

  function groupTargets(targets, groupingValue = "type") {
    const grouping = normalizeResultGrouping(groupingValue);
    if (grouping === "none") return [];
    const groupsById = new Map();
    for (const target of Array.isArray(targets) ? targets : []) {
      const descriptor = resultGroupDescriptor(target, grouping);
      if (!descriptor) continue;
      if (!groupsById.has(descriptor.id)) groupsById.set(descriptor.id, { ...descriptor, targets: [] });
      groupsById.get(descriptor.id).targets.push(target);
    }
    const groups = Array.from(groupsById.values());
    groups.sort((left, right) => left.rank - right.rank || left.label.localeCompare(right.label) || left.id.localeCompare(right.id));
    return groups.map((group) => ({ id: group.id, label: group.label, targets: group.targets }));
  }
  ...
  class QuickApplyDomain {
    constructor(options = {}) {
      ...
      this.preferences = {
        ...
        grouping: normalizeResultGrouping(options.preferences && options.preferences.grouping),
    ...
    setVisible(value) {
      return this.setLifecycle({ visible: value, active: this.active });
    }

    setActive(value) {
      return this.setLifecycle({ visible: this.visible, active: value });
    }

    setLifecycle(value = {}, options = {}) {
      if (this.destroyed) return false;
      const nextVisible = Boolean(value.visible);
      const nextActive = Boolean(value.active);
      const visibleChanged = nextVisible !== this.visible;
      const activeChanged = nextActive !== this.active;
      if (!visibleChanged && !activeChanged) return false;
      this.visible = nextVisible;
      this.active = nextActive;
      this.forwardLifecycle({ visible: visibleChanged, active: activeChanged });
      if (options.emit !== false) this.emit();
      return true;
    }

    forwardLifecycle(changed = { visible: true, active: true }) {
      if (changed.visible !== false && typeof this.adapter.setVisible === "function") this.adapter.setVisible(this.visible);
      if (changed.active !== false && typeof this.adapter.setActive === "function") this.adapter.setActive(this.active);
    }

    setPreferences(value = {}) {
      this.preferences = {
        favoritesFirst: value.favoritesFirst !== false,
        grouping: normalizeResultGrouping(value.grouping, this.preferences.grouping),
    ...
    visibleTargets() {
      ...
      const visible = scored.slice(0, this.maxResults).map((entry) => entry.target);
      const groups = groupTargets(visible, this.preferences.grouping);
      return groups.length ? groups.flatMap((group) => group.targets) : visible;
    }
    ...
    getSnapshot() {
      ...
      if (!message) {
        message = ({
          ...
          ready: `${targets.length} supported effect${targets.length === 1 ? "" : "s"} and Blocky Studios Recipe result${targets.length === 1 ? "" : "s"}.`,
      ...
      return {
        ...
        query: this.query,
        scope: this.scope,
        grouping: this.preferences.grouping,
        targets,
        targetGroups: groupTargets(targets, this.preferences.grouping),
        selectedTarget,
    ...
    persistState(reason) {
      if (this.persistenceBlocked) {
        return Promise.reject(new Error("[METADATA_TRANSACTION_ACTIVE] Quick Apply metadata is being replaced."));
      }
    ...
    setPersistenceBlocked(value) {
      this.persistenceBlocked = Boolean(value);
    ...
    toggleFavorite(id = this.selectedTargetId) {
      ...
      if (target.kind === "recipe") {
        ...
        void this.persistRecipes("quick-apply-recipe-favorite").catch((error) => {
          ...
          this.actionStatus = { ...persistenceFailure(error, "Blocky Studios could not save this recipe favorite."), tone: "error", targetId: target.id, at: this.now() };
          this.emit();
        });
      } else {
        ...
        void this.persistState("quick-apply-effect-favorite").catch((error) => {
          ...
          this.actionStatus = { ...persistenceFailure(error, "Blocky Studios could not save this effect favorite."), tone: "error", targetId: target.id, at: this.now() };
          this.emit();
        });
      }
      this.emit();
    ...
    async applyTarget(id = this.selectedTargetId) {
      const target = this.getTarget(id);
      if (!target) return { ok: false, code: "NO_TARGET", message: "Choose a supported effect or Blocky Studios Recipe first." };
      ...
      try {
        ...
        if (target.kind === "effect") {
          ...
          const plan = await this.adapter.planEffectApplication(target.effect, {
            applyOnce: true,
            undoString: `Blocky Studios Quick Apply: ${target.name}`,
          ...
          if (typeof this.adapter.planRecipeApplication !== "function" || typeof this.adapter.applyRecipe !== "function") {
            throw new Error("Transactional Blocky Studios Recipe application is unavailable in this Premiere runtime.");
          }
          const plan = await this.adapter.planRecipeApplication(target.recipe, {
            applyOnce: request.applyOnce,
            undoString: `Blocky Studios Recipe: ${target.name}`,
            parameterUndoString: `Blocky Studios Recipe Parameters: ${target.name}`,
            selectionRevision: request.selectionRevision,
          });
        ...
        if (result.historyEligible === true) {
          try {
            ...
            historyPersistence = persistenceFailure(error, `${target.name} was applied and verified, but Blocky Studios could not save its Recent history.`).persistence;
          }
        }
    ...
    async getSupportedParameters(effectId) {
      ...
      if (!response || !isPlainObject(response.capability) || response.capability.supported !== true) return [];
      const values = Array.isArray(response) ? response : response && Array.isArray(response.parameters) ? response.parameters : [];
      return (Array.isArray(values) ? values : [])
        .filter((value) => value && value.supported === true)
    ...
    async saveRecipe(draft = {}, existingId = "") {
      ...
      if (this.library.recipes.some((entry, index) => index !== existingIndex && entry.name.toLocaleLowerCase("en-US") === recipe.name.toLocaleLowerCase("en-US"))) {
        return { ok: false, code: "DUPLICATE_RECIPE_NAME", message: "An Blocky Studios Recipe already uses this name." };
      }
      const previousLibrary = clone(this.library);
      ...
      try {
        ...
        this.library = previousLibrary;
        const failure = persistenceFailure(error, "Blocky Studios could not save this recipe.");
    ...
    async renameRecipe(recipeId, name) {
      ...
      if (!recipe) return { ok: false, code: "RECIPE_NOT_FOUND", message: "The Blocky Studios Recipe no longer exists." };
    ...
    async deleteRecipe(recipeId) {
      ...
      if (index < 0) return { ok: false, code: "RECIPE_NOT_FOUND", message: "The Blocky Studios Recipe no longer exists." };
      const previousLibrary = clone(this.library);
      ...
      try {
        ...
        this.library = previousLibrary;
        const failure = persistenceFailure(error, "Blocky Studios could not delete this recipe.");
    ...
    async moveRecipe(recipeId, delta) {
      ...
      try {
        ...
        this.library = previousLibrary;
        const failure = persistenceFailure(error, "Blocky Studios could not save the recipe order.");
        this.actionStatus = { ...failure, tone: "error", targetId: `recipe:${recipeId}`, at: this.now() };
        this.emit();
        return failure;
      }
    ...
    async importRecipes(serialized, options = {}) {
      try {
        ...
        try {
          ...
          this.library = previousLibrary;
          const failure = persistenceFailure(error, "Blocky Studios could not save the imported recipes.");
          this.actionStatus = { ...failure, tone: "error", at: this.now() };
          this.emit();
          return failure;
        }
        this.actionStatus = { ok: true, verified: true, tone: "success", message: `${imported} Blocky Studios Recipe${imported === 1 ? "" : "s"} imported.`, at: this.now() };
        this.emit();
        return { ok: true, imported };
      } catch (error) {
        const result = { ok: false, code: "RECIPE_IMPORT_FAILED", message: cleanText(error && error.message, 2048) || "Blocky Studios Recipe import failed." };
        this.actionStatus = { ...result, tone: "error", at: this.now() };
        this.emit();
        return result;
      }
  ...
  return Object.freeze({
    ...
    RESULT_GROUPINGS: RESULT_GROUPINGS.slice(),
    MAX_INDEX_EFFECTS,
    MAX_RESULTS,
    MAX_RECIPES,
    MAX_RECIPE_STEPS,
