...
(function exposeOracleQuickApplyDomain(globalScope, factory) {
  ...
  function importRecipeLibrary(serialized, options = {}) {
    ...
    if (!text || utf8ByteLength(text) > MAX_IMPORT_BYTES) throw new TypeError("Oracle Recipe import is empty or exceeds the 2 MB limit.");
    let parsed;
  ...
  function resultGroupDescriptor(target, grouping) {
    ...
    if (target && target.kind === "recipe") {
      return { id: "recipes", label: "Oracle Recipes", rank: 10000 };
    }
    if (grouping === "type") {
