...
(function exposeOraclePreferences(globalScope, factory) {
  ...
  function validatePreferences(preferences) {
    ...
    const curveContrast = contrastRatio(normalized.curves.curveColor, normalized.appearance.background);
    if (curveContrast < 3) {
      errors.push("Curve color needs at least 3:1 contrast against the workspace background.");
    }
    const curveGridContrast = contrastRatio(normalized.curves.gridColor, normalized.appearance.background);
    if (curveGridContrast < 1.15) {
      errors.push("Curve grid color is too close to the workspace background to remain visible.");
    }
  ...
  async function readEntryByteSize(entry) {
    if (!entry || (typeof entry !== "object" && typeof entry !== "function")) {
      throw new Error("Oracle could not verify the selected file size before reading it.");
    }
    let rawSize;
  ...
  class OraclePreferencesController {
    ...
    async performAction(action) {
      ...
      try {
        ...
        if (action === "reset-section") {
          if (this.activeSection === "data") {
            ...
            if (!result || result.cancelled !== true) {
              ...
              this.onToast("Oracle metadata reset. This metadata action did not delete source media.", "success");
            }
            return;
          }
          const defaults = createDefaultPreferences();
        ...
        if (action === "export-theme") {
          const entry = await this.chooseEntry("save", { suggestedName: "oracle-theme.json", types: ["json"] });
          this.assertOperationCurrent(generation);
          if (!entry) return;
        ...
        if (action === "export-settings") {
          const entry = await this.chooseEntry("save", { suggestedName: "oracle-settings.json", types: ["json"] });
          this.assertOperationCurrent(generation);
          if (!entry) return;
        ...
        if (action === "export-metadata") {
          ...
          if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
            throw new Error("Oracle could not create a valid metadata export.");
          }
          const text = JSON.stringify(payload, null, 2);
          if (!text) throw new Error("Oracle could not serialize replay metadata.");
          const entry = await this.chooseEntry("save", { suggestedName: "oracle-replay-metadata.json", types: ["json"] });
          this.assertOperationCurrent(generation);
          if (!entry) return;
        ...
        if (action === "support-bundle") {
          const entry = await this.chooseEntry("save", { suggestedName: "oracle-support.json", types: ["json"] });
          this.assertOperationCurrent(generation);
          if (!entry) return;
    ...
    showSection(section) {
      ...
      if (resetButton) {
        ...
        resetButton.title = section === "data"
          ? "Reset Oracle metadata after an explicit confirmation. This metadata action does not delete source media."
    ...
    updateContrastStatus() {
      ...
      if (status) {
        ...
        const accent = contrastRatio(values.appearance.accent, values.appearance.background);
      ...
      if (curvesStatus) {
        const line = contrastRatio(values.curves.curveColor, values.appearance.background);
        const grid = contrastRatio(values.curves.gridColor, values.appearance.background);
        const separation = contrastRatio(values.curves.curveColor, values.curves.gridColor);
        curvesStatus.textContent = `Curve ${line.toFixed(2)}:1 · Grid ${grid.toFixed(2)}:1 · Separation ${separation.toFixed(2)}:1`;
