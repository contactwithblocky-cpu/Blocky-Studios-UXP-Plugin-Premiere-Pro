...
(function exposeOracleRuntimeRegistry(globalScope, factory) {
  ...
  class RuntimeCoordinationError extends Error {
    constructor(code, message, details = {}) {
      super(message || code || "Oracle runtime coordination failed.");
  ...
  class DiagnosticOwner {
    ...
    recordDiagnostic(code, details = {}) {
      ...
      if (this.logger && typeof this.logger.warn === "function") {
        try { this.logger.warn(`[Oracle Runtime][${code}]`, details); } catch (error) { /* diagnostics stay non-fatal */ }
      }
      return entry;
    }
    ...
    assertOwner(record) {
      ...
      if (this.owner !== record || !record.token) {
        throw new RuntimeCoordinationError(
          ...
          "Another Oracle panel owns the Source Monitor viewer.",
