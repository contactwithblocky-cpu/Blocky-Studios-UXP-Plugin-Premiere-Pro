...
(function exposeOracleCurvesWorkspace(globalScope, factory) {
  ...
  const PRESET_ACTIONS = Object.freeze([
    ...
    "moveUp", "moveDown", "favorite", "import", "export",
  ]);
  ...
  class CurvesWorkspaceController {
    ...
    setPreferences(value) {
      ...
      if (this.elements.grid) this.elements.grid.hidden = !this.preferences.gridVisible;
      this.sync();
      this.scheduleRender();
    }
      ...
      if (this.elements.samplePreview) {
        if (this.mode !== "baked") this.elements.samplePreview.textContent = "No generated keys in Native mode";
          ...
          try {
            ...
            this.elements.samplePreview.textContent = `${preview.addedKeyCount} generated key${preview.addedKeyCount === 1 ? "" : "s"} · ${preview.collisionCount} preserved collision${preview.collisionCount === 1 ? "" : "s"}`;
          } catch (error) {
    ...
    async invokePresetAction(action) {
      ...
      if (["rename", "overwrite", "delete", "moveUp", "moveDown"].includes(action) && (!selected || this.presetTab !== "user")) return false;
