...
(function exposeOracleReplayLibrary(globalScope, factory) {
  ...
  function physicalFingerprint(record) {
    ...
    return identity
      ? `${path}|identity:${identity}|${observation}`
      : `${path}|${observation}`;
  }
  ...
  class ReplayLibraryStore {
    ...
    ingest(message, options = {}) {
      ...
      if (repeatedId && workingState.replaysById[repeatedId]) {
        ...
        const identityChanged = previousIdentity && incomingIdentity && previousIdentity !== incomingIdentity;
        ...
        if (pathChanged || identityChanged || sizeChanged) {
          return {
            ok: false,
  ...
  function thumbnailCacheKey(record, options = {}) {
    return hashKey(stableJson({
      ...
      width: Math.round(finite(options.width, 640, 64, MAX_THUMBNAIL_DIMENSION)),
      height: Math.round(finite(options.height, 360, 64, MAX_THUMBNAIL_DIMENSION)),
    }));
  }
  ...
  class ThumbnailCache {
      ...
      try {
        ...
        if (payload && payload.schema === "oracle-thumbnail-cache" && payload.version === THUMBNAIL_SCHEMA_VERSION) {
          ...
          for (const entry of entries) {
            ...
            if (!normalized || this.entries.has(normalized.key)) continue;
    ...
    mutate(task) {
      this.mutationChain = this.mutationChain.catch(() => undefined).then(task);
      return this.mutationChain;
    }

    assertCurrent(generation) {
      if (this.destroyed || generation !== this.generation) {
        throw new Error("Thumbnail cache operation was cancelled.");
      }
    }
    ...
    store(record, base64, options = {}) {
      ...
      return this.queue.submit(async () => {
        ...
        await this.load();
        if (!this.fs || typeof this.fs.writeFile !== "function" || typeof this.fs.rename !== "function") {
          throw new Error("Permanent thumbnail storage is unavailable.");
        }
        const bytes = decodeBase64(base64);
        if (bytes.byteLength > MAX_THUMBNAIL_BYTES) throw new Error("Thumbnail exceeds the 8 MB cache limit.");
        ...
        return this.mutate(async () => {
          ...
          if (existing) {
            ...
            await this.evictUnlocked(options.limitMb || 512);
            this.assertCurrent(generation);
            await this.persistIndexUnlocked();
            return { key, url, bytes: existing.bytes, reused: true };
          }
          const temp = `${url}.tmp`;
          ...
          try {
            ...
            this.assertCurrent(generation);
            await this.fs.writeFile(temp, bytes);
            this.assertCurrent(generation);
          ...
          this.entries.set(key, {
            ...
            pathKeyHash: hashKey(record.pathKey),
            sourceFingerprint: hashKey(physicalFingerprint(record)),
          });
          await this.evictUnlocked(options.limitMb || 512);
          this.assertCurrent(generation);
          await this.persistIndexUnlocked();
          return { key, url, bytes: bytes.byteLength, reused: false };
        });
      });
    }

    async evictUnlocked(limitMb) {
      const limit = Math.max(1, finite(limitMb, 512, 1, 4096)) * 1024 * 1024;
    ...
    async invalidate(record) {
      ...
      return this.mutate(async () => {
        this.assertCurrent(generation);
        const pathHash = hashKey(record.pathKey);
