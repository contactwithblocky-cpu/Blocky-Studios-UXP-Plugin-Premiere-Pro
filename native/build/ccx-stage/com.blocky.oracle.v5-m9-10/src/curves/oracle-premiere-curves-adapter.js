...
(function exposeOraclePremiereCurvesAdapter(globalScope, factory) {
  ...
  const REASON = Object.freeze({
    ...
    POINTF_INTERPOLATION_READBACK_UNAVAILABLE: "POINTF_INTERPOLATION_READBACK_UNAVAILABLE",
  ...
  const REASON_MESSAGES = Object.freeze({
    ...
    [REASON.NO_SELECTION]: "Select one or more timeline clips to inspect their keyframes.",
    ...
    [REASON.POINTF_INTERPOLATION_READBACK_UNAVAILABLE]: "Premiere 26.3 exposes PointF key values but not verifiable temporal interpolation readback, so PointF Apply is disabled.",
    [REASON.UNPROVEN_TIME_BASIS]: "Curves Apply is verified only for an untrimmed, zero-start, forward 1x clip because Premiere 26.3 does not expose a safe keyframe time-basis conversion for this clip.",
    ...
    [REASON.BAKED_RUNTIME_PROOF_REQUIRED]: "Baked Oracle curves remain disabled until generated-key actions, interpolation, readback, and one-step Undo pass the runtime proof.",
    [REASON.CLEANUP_OWNERSHIP_REQUIRED]: "Oracle cannot remove keys without an exact ownership receipt.",
    [REASON.PRIOR_BAKE_REPLACEMENT_UNPROVEN]: "A prior Oracle-owned bake overlaps this property segment. Oracle retained its exact receipt, but same-transaction remove-and-replace ordering is not physically proven in this Premiere runtime, so replacement is blocked without changing the project.",
  });
  ...
  function stateForReason(code) {
    ...
    if ([REASON.NO_ACTIVE_PROJECT, REASON.NO_ACTIVE_SEQUENCE, REASON.NO_SELECTION, REASON.NO_KEYFRAMES,
  ...
  async function readTrackDescriptor(api, item) {
    ...
    return {
      ...
      descriptor: Object.freeze({
        ...
        timeBasisProven: startTicks === "0" && inPointTicks === "0" && outPointTicks !== null &&
          outPointTicks === durationTicks && Number(speed) === 1 &&
  ...
  function publicBinding(hostBinding, nativeModes, bakedRuntimeProof) {
    ...
    const endpointKind = endpoints && endpoints.start && endpoints.start.valueKind;
    const nativeCapability = hostBinding.identity.track.timeBasisProven !== true
      ? reason(REASON.UNPROVEN_TIME_BASIS)
      : endpointKind === "pointf"
      ? reason(REASON.POINTF_INTERPOLATION_READBACK_UNAVAILABLE)
      : !hostBinding.bracket.hasSegment
    ...
    const bakedCapability = bakedKindProven
      ...
      : hostBinding.identity.track.timeBasisProven !== true
        ...
        : reason(endpointKind === "pointf"
          ? REASON.POINTF_INTERPOLATION_READBACK_UNAVAILABLE
          : REASON.VALUE_KIND_UNSUPPORTED, { valueKindSupported: false });
  ...
  async function inspectTrackItem(api, context, item, nativeModes, bakedRuntimeProof, limits) {
      ...
      try {
        ...
        for (let paramIndex = 0; paramIndex < paramCount; paramIndex += 1) {
          try {
            ...
            for (const raw of Array.isArray(rawTimes) ? rawTimes : []) {
            ...
            const bracket = bracketTickTimes(timeRecords.map((entry) => entry.ticks), context.playheadTicks);
            ...
            bindings.push({
              ...
              display: {
                ...
                property: cleanText(param.displayName, 512) || `Property ${paramIndex + 1}`,
  ...
  class PremiereCurvesAdapter {
    ...
    publish(snapshot) {
      ...
      for (const listener of Array.from(this.subscribers)) {
        try { listener(snapshot); } catch (error) {
          if (this.logger && typeof this.logger.error === "function") this.logger.error("[Oracle Curves] Snapshot subscriber failed.", error);
        }
      }
    }
    ...
    bindSequenceEvents(sequence) {
      ...
      for (const eventName of eventNames) {
        try {
          ...
          if (this.logger && typeof this.logger.warn === "function") this.logger.warn("[Oracle Curves] Sequence event unavailable.", error);
        }
      }
    ...
    async inspectHost() {
      ...
      if (typeof sequence.getSelection !== "function" || typeof sequence.getPlayerPosition !== "function") {
        return { snapshot: this.makeStatusSnapshot(REASON.SELECTION_UNREADABLE), descriptors: new Map(), refs: new Map(), project, sequence };
      }
      const [selection, playhead] = await Promise.all([sequence.getSelection(), sequence.getPlayerPosition()]);
      ...
      if (items.length === 0) {
        const snapshot = this.makeStatusSnapshot(REASON.NO_SELECTION, {
      ...
      for (let itemIndex = 0; itemIndex < items.length; itemIndex += 1) {
        try {
          const inspected = await inspectTrackItem(api, context, items[itemIndex], nativeModes, this.bakedRuntimeProof, this.limits);
      ...
      const selectionStatus = publicBindings.length > 0 ? reason(REASON.OK) : reason(REASON.KEYFRAMES_UNSUPPORTED);
    ...
    async planNativeInterpolation(bindingOrBindings, modeName, options = {}) {
      ...
      const planCore = {
        ...
        undoString: cleanText(options.undoString, 128) || `Oracle Curves: ${name[0]}${name.slice(1).toLowerCase()}`,
    ...
    normalizeBakedCandidates(publicBinding, candidates) {
      ...
      if (!start || !end || start.valueKind !== "number" || end.valueKind !== "number") {
        throw new CurvesAdapterError(
          start && start.valueKind === "pointf"
            ? REASON.POINTF_INTERPOLATION_READBACK_UNAVAILABLE
            : REASON.VALUE_KIND_UNSUPPORTED,
          undefined,
          { bindingId: publicBinding.bindingId },
        );
      }
      ...
      if (interior.length === 0) {
        throw new CurvesAdapterError(REASON.INVALID_PLAN, "Baked Oracle curves require at least one new frame-aligned interior key.", {
          bindingId: publicBinding.bindingId,
          emptyBake: true,
        });
      }
    ...
    async planBakedCurve(bindingOrBindings, generatedKeysByBinding = {}, options = {}) {
      ...
      const planCore = {
        ...
        projectGuid: inspection.snapshot.project.guid,
        sequenceGuid: inspection.snapshot.sequence.guid,
        undoString: cleanText(options.undoString, 128) || "Oracle Curves: Bake Curve",
    ...
    async cleanupOwnedKeys(receipt, options = {}) {
      ...
      if (actionFactories.length > 0) {
        this.executeLockedActions(
          inspection.project,
          cleanText(options.undoString, 128) || "Oracle Curves: Remove Baked Keys",
          actionFactories,
        );
      }
      for (const entry of resolvedOperations) {
