...
if (!schema) throw new Error("Oracle data schema did not load before migrations.");
const {
  EMPTY_TIMESTAMP,
  ORACLE_STATE_SCHEMA,
...
class OracleStateRecoveryError extends Error {
  constructor(diagnostics, sources) {
    super("Oracle state files exist, but none can be recovered safely. Preserve the files and restore a valid backup before continuing.");
  ...
  if (evaluated.length === 0) {
    if (sourceCandidates.length > 0) {
      const diagnostics = rejectedDiagnostics.concat({
        ...
        message: "Oracle state candidates exist, but every candidate is invalid or incomplete.",
    ...
    return {
      ...
      diagnostics: [{ code: "NO_STATE_CANDIDATES", message: "No Oracle state snapshot exists yet" }],
      recovered: false,
      firstRun: true,
      candidateCount: 0,
    };
