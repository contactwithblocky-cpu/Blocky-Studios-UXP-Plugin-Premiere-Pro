...
const NATIVE_ADDON_BOOTSTRAP_TIMEOUT_MS = 2000;
const ORACLE_PLUGIN_VERSION = "2.0.18";
...
/** @type {Window & typeof globalThis & { bridgeStatus: BridgeStatus, oracleDiagnostics?: any, oraclePlatformTelemetry?: any, oraclePlatformAudit?: any, oracleNativeDragAddon?: any, oracleNativeDragDiagnostics?: any, oracleWorkspacePreferences?: any, oracleUiRuntime?: any, oracleUiHealth?: any, oracleButtonPaintBridge?: any, OracleBuildInfo?: any, OracleOverdrivePreferences?: any, OracleOverdriveShell?: any, OraclePanelDom?: any, OracleRuntimeRegistry?: any, OracleReplayLibrary?: any, OracleReplayMedia?: any, OracleReplayWorkspace?: any, OracleReplayOrganization?: any, OracleReplayLifecycleUI?: any, OracleReplayViewer?: any, OracleCurveMath?: any, OracleCurvePresets?: any, OraclePremiereCurvesAdapter?: any, OracleCurvesWorkspace?: any, OracleEffectIndex?: any, OraclePremiereEffectsAdapter?: any, OracleQuickApplyDomain?: any, OracleQuickApplyWorkspace?: any }} */
...
const oracleDiagnostics = oracleWindow.oracleDiagnostics &&
  ...
  : null;
const oraclePlatformTelemetry = /** @type {any} */ (oracleWindow).oraclePlatformTelemetry || null;

function tracePlatformCall(category, name, operation, details = {}) {
  if (oraclePlatformTelemetry && typeof oraclePlatformTelemetry.invoke === "function") {
    return oraclePlatformTelemetry.invoke(category, name, operation, details);
  }
  return operation();
}

function tracePremiereCall(name, operation, details = {}) {
  return tracePlatformCall("premiere", name, operation, details);
}

function traceNativeCall(name, operation, details = {}) {
  return tracePlatformCall("native-addon", name, operation, details);
}

function traceTabSwitch(details = {}) {
  if (!oraclePlatformTelemetry || typeof oraclePlatformTelemetry.tabSwitch !== "function") return null;
  return oraclePlatformTelemetry.tabSwitch(details);
}
...
function oracleErrorMessage(error, fallback = "Blocky Studios operation failed.") {
...
function reportOracleCritical(code, error, details = {}) {
  ...
  const safeMessage = entry && entry.details && entry.details.message
    ...
    : "Blocky Studios encountered a critical error.";
  console.error(`[Blocky Studios][${safeCode}] ${safeMessage}`);
  return entry;
}

let premiere = null;
try {
  premiere = tracePremiereCall("require(premierepro)", () => require("premierepro"));
} catch (error) {
...
function nativeAddonFailure(errorCode, errorMessage, runtimeInfo, exception, extra = {}) {
  ...
  if (typeof reportOracleCritical === "function") {
    ...
    console.error(`[Blocky Studios][${String(errorCode || "ADDON_FAILURE")}] Native addon initialization failed.`);
  }
  return failure;
}
...
async function loadNativeDragAddon() {
  ...
  try {
    ...
    addon = await traceNativeCall("require(oracle-native-drag.uxpaddon)", () => require(NATIVE_DRAG_ADDON_NAME));
  } catch (error) {
    return {
      addon: null,
  ...
  if (
    ...
    !("prepareReplayMedia" in Object(addon)) ||
    !("cancelReplayMedia" in Object(addon))
  ) {
    return {
      addon: null,
      diagnostic: nativeAddonFailure(
  ...
  if (
    ...
    typeof addon.prepareReplayMedia !== "function" ||
    typeof addon.cancelReplayMedia !== "function"
  ) {
    return {
      addon: null,
      diagnostic: nativeAddonFailure(
        ...
        null,
        {
          ...
          prepareReplayMediaType: typeof addon.prepareReplayMedia,
          cancelReplayMediaType: typeof addon.cancelReplayMedia,
        },
      ),
    };
  }
  ...
  let selfTest;
  try {
    selfTest = traceNativeCall("nativeSelfTest", () => addon.nativeSelfTest());
  } catch (error) {
    return {
      addon: null,
      diagnostic: nativeAddonFailure(
  ...
  if (
    ...
    selfTest.replayMediaPreparationAvailable !== true
  ) {
    return {
      addon: null,
      diagnostic: nativeAddonFailure(
  ...
  let packagedFonts;
  try {
    packagedFonts = traceNativeCall("registerPackagedFonts", () => addon.registerPackagedFonts());
  } catch (error) {
    return {
      addon: null,
      diagnostic: nativeAddonFailure(
...
async function loadNativeDragAddonForBootstrap(loadPromise, timeoutMs = NATIVE_ADDON_BOOTSTRAP_TIMEOUT_MS) {
  const boundedTimeout = Math.max(250, Number(timeoutMs) || NATIVE_ADDON_BOOTSTRAP_TIMEOUT_MS);
  let timeoutHandle = null;
  const timeoutResult = new Promise((resolve) => {
    timeoutHandle = setTimeout(() => {
      nativeAddonBootstrapTimedOut = true;
      const diagnostic = nativeDragError(
        "ADDON_LOAD_TIMEOUT",
        `Native addon loading exceeded the ${boundedTimeout} ms UI bootstrap limit.`,
        Object.assign({}, nativeDragRuntimeInfo(), {
          timeoutMs: boundedTimeout,
          timedOut: true,
        }),
      );
      recordOracleDiagnostic("warn", "ADDON_LOAD_TIMEOUT", {
        timeoutMs: boundedTimeout,
        baselineUiContinuing: true,
      });
      resolve({ addon: null, diagnostic });
    }, boundedTimeout);
  });
  try {
    return await Promise.race([loadPromise, timeoutResult]);
  } finally {
...
let nativeAddonBootstrapTimedOut = false;
let nativeAddonLateRecoveryScheduled = false;

function commitNativeDragAddonLoadResult(loadResult) {
...
const nativeDragAddonLoadCompletionPromise = loadNativeDragAddon().then((loadResult) => {
  commitNativeDragAddonLoadResult(loadResult);
  if (
    nativeAddonBootstrapTimedOut &&
    loadResult &&
    loadResult.addon &&
    !nativeAddonLateRecoveryScheduled
  ) {
    nativeAddonLateRecoveryScheduled = true;
    recordOracleDiagnostic("info", "NATIVE_ADDON_LATE_RECOVERY", {
      bootstrapReloadScheduled: true,
      nativeAddonReady: true,
    });
    Promise.resolve().then(() => scheduleOraclePanelInitialization());
  }
  return loadResult;
});

const nativeDragAddonLoadPromise = loadNativeDragAddonForBootstrap(
  nativeDragAddonLoadCompletionPromise,
).then((loadResult) => {
  if (!nativeDragAddon || (loadResult && loadResult.addon)) {
    commitNativeDragAddonLoadResult(loadResult);
  }
  return loadResult;
});
...
try {
  ...
  // URL keeps history inside the Blocky Studios persistent sandbox without user prompts.
  ...
  if (!oracleDataFolderNativePathPromise) {
    oracleDataFolderNativePathPromise = (async () => {
      ...
      if (!localFileSystem || typeof localFileSystem.getDataFolder !== "function") {
        throw new Error("UXP did not expose the Blocky Studios plugin-data folder.");
      }
...
function createNativeStateAtomicWriter(addon) {
  ...
  return async ({ text }) => {
    ...
    const result = await traceNativeCall("writeAtomicStateFile", () => addon.writeAtomicStateFile(
...
function logTimelineLabelTelemetry(eventName, details = {}) {
  ...
  if (NATIVE_DRAG_DEBUG) {
    console.log(`[Blocky Studios Timeline Label][${record.event}]`, record);
  }
  return record;
}
...
class OracleLogoAnimator {
  ...
  start() {
    ...
    // The supplied Blocky Studios artwork is a normal responsive image. Keep
    // the legacy class seam but do no resize work and schedule no frame loop.
...
function updateBridgeStatus(status) {
  ...
  if (indicator) {
    ...
    indicator.title = connected
      ? "Connected to the local Blocky Studios bridge"
      : status === "connecting"
        ? "Connecting to the local Blocky Studios bridge"
        : "Disconnected from the local Blocky Studios bridge";
...
class OracleBridgeClient {
  ...
  persistLifecycleReconciliations() {
    ...
    try {
      ...
      this.lifecyclePersistenceWarning =
        "Replay-path bridge delivery could not be saved for panel restart. Keep this panel open until Blocky Studios acknowledges the move.";
      return false;
    }
  }
  ...
  disposeSocket(socket, closeSocket) {
    ...
    if (closeSocket && (socket.readyState === 0 || socket.readyState === 1)) {
      try {
        socket.close(1000, "Blocky Studios bridge reconnect");
      } catch (error) {
...
class PremiereGateway {
  ...
  async importReplay(filepath, suppressionOverride = undefined) {
    ...
    const project = await tracePremiereCall("Project.getActiveProject", () => this.api.Project.getActiveProject());
    if (!project) {
  ...
  async performImport(project, filepath, caller = "unknown", suppressionOverride = undefined) {
    ...
    const root = await tracePremiereCall("Project.getRootItem", () => project.getRootItem());
    ...
    const destinationProjectItem = tracePremiereCall("ProjectItem.cast", () => this.api.ProjectItem.cast(importDestination));
    let imported = false;
    try {
  ...
  async getOrCreateReplayBin(project, root) {
    const rootFolder = tracePremiereCall("FolderItem.cast", () => this.api.FolderItem.cast(root));
    const findReplayBin = async () => {
      const children = await Promise.resolve(tracePremiereCall("FolderItem.getItems", () => rootFolder.getItems()));
    ...
    const creation = (async () => {
      ...
      try {
        tracePremiereCall("Project.lockedAccess", () => project.lockedAccess(() => {
          transactionSucceeded = tracePremiereCall("Project.executeTransaction", () => project.executeTransaction((compoundAction) => {
            const createBinAction = tracePremiereCall("FolderItem.createBinAction", () => rootFolder.createBinAction(PREMIERE_REPLAY_BIN_NAME, true));
            tracePremiereCall("CompoundAction.addAction", () => compoundAction.addAction(createBinAction));
      ...
      tracePremiereCall("Project.lockedAccess", () => project.lockedAccess(() => {
        transactionSucceeded = tracePremiereCall("Project.executeTransaction", () => project.executeTransaction((compoundAction) => {
          ...
          for (const action of actions || []) {
            tracePremiereCall("CompoundAction.addAction", () => compoundAction.addAction(action));
          }
        }, undoString));
      }));
    } catch (error) {
  ...
  async scanProjectItemsByExactMediaPath(root, filepath, options = {}) {
    ...
    while (queueIndex < queue.length && visited < visitLimit) {
      ...
      let children;
      try {
        children = await Promise.resolve(tracePremiereCall("FolderItem.getItems", () => parent.getItems()));
      } catch (error) {
        if (typeof recordOracleDiagnostic === "function") {
      ...
      for (const child of children || []) {
        ...
        if (!isFolder) {
          try {
            const clipItem = tracePremiereCall("ClipProjectItem.cast", () => this.api.ClipProjectItem.cast(child));
            const mediaPath = await tracePremiereCall("ClipProjectItem.getMediaFilePath", () => clipItem.getMediaFilePath());
        ...
        if (isFolder) {
          try {
            queue.push(tracePremiereCall("FolderItem.cast", () => this.api.FolderItem.cast(child)));
          } catch (error) {
            skippedItems += 1;
          }
        }
  ...
  async applyPremiereLabelValue9(
    ...
    if (!sourceLabelAlreadyApplied && typeof projectItem.getColorLabelIndex === "function") {
      sourceLabelAlreadyApplied =
        await tracePremiereCall("ProjectItem.getColorLabelIndex", () => projectItem.getColorLabelIndex()) === PREMIERE_REPLAY_LABEL_VALUE;
    }
    ...
    if (!sourceLabelAlreadyApplied) {
      const committed = this.executeLockedTransaction(
        project,
        "Apply Blocky Studios replay label 9",
        () => [tracePremiereCall("ProjectItem.createSetColorLabelAction", () => projectItem.createSetColorLabelAction(PREMIERE_REPLAY_LABEL_VALUE))],
      );
      if (!committed) {
        throw new UserFacingError(
    ...
    if (typeof projectItem.getColorLabelIndex === "function") {
      const appliedValue = await tracePremiereCall("ProjectItem.getColorLabelIndex", () => projectItem.getColorLabelIndex());
  ...
  async performReplayInsert(replayPayload) {
    ...
    const projectItem = tracePremiereCall("ProjectItem.cast", () => this.api.ProjectItem.cast(clipItem));
    const projectItemId = String(await Promise.resolve(tracePremiereCall("ProjectItem.getId", () => projectItem.getId())));
    const playhead = await tracePremiereCall("Sequence.getPlayerPosition", () => sequence.getPlayerPosition());
    const insertedAtTicks = tickTimeTicks(playhead);
    const editor = tracePremiereCall("SequenceEditor.getEditor", () => this.api.SequenceEditor.getEditor(sequence));
    const videoTrackCount = Math.max(0, Number(await tracePremiereCall("Sequence.getVideoTrackCount", () => sequence.getVideoTrackCount())) || 0);
    const audioTrackCount = Math.max(0, Number(await tracePremiereCall("Sequence.getAudioTrackCount", () => sequence.getAudioTrackCount())) || 0);
    ...
    for (const attempt of attempts) {
      try {
        const committed = this.executeLockedTransaction(
          project,
          `Insert Blocky Studios replay: ${cleanTitle(replayPayload.title)}`,
          () => [
            tracePremiereCall("SequenceEditor.createInsertProjectItemAction", () => editor.createInsertProjectItemAction(
              projectItem,
              playhead,
              attempt.videoTrackIndex,
              attempt.audioTrackIndex,
              false,
            ), { videoTrackIndex: attempt.videoTrackIndex, audioTrackIndex: attempt.audioTrackIndex }),
          ],
        );
        if (committed) {
  ...
  async findInsertedTrackItems(sequence, projectItemId, playhead, trackPlan) {
    ...
    const candidates = [];
    try {
      const videoTrack = await tracePremiereCall("Sequence.getVideoTrack", () => sequence.getVideoTrack(trackPlan.videoTrackIndex), { trackIndex: trackPlan.videoTrackIndex });
      candidates.push(
    ...
    try {
      const audioTrack = await tracePremiereCall("Sequence.getAudioTrack", () => sequence.getAudioTrack(trackPlan.audioTrackIndex), { trackIndex: trackPlan.audioTrackIndex });
      candidates.push(
        ...(await Promise.resolve(
    ...
    for (const trackItem of candidates) {
      try {
        const sourceItem = await tracePremiereCall("TrackItem.getProjectItem", () => trackItem.getProjectItem());
        const sourceId = String(await Promise.resolve(sourceItem.getId()));
        const startTicks = tickTimeTicks(await tracePremiereCall("TrackItem.getStartTime", () => trackItem.getStartTime()));
  ...
  async openInSourceMonitor(replay) {
    ...
    const projectItem = tracePremiereCall("ProjectItem.cast", () => this.api.ProjectItem.cast(clipItem));
    const opened = await tracePremiereCall("SourceMonitor.openProjectItem", () => this.api.SourceMonitor.openProjectItem(projectItem));
    if (!opened) {
  ...
  async setSourceMonitorPlayback(replay, playing) {
    ...
    const accepted = await tracePremiereCall("SourceMonitor.play", () => this.api.SourceMonitor.play(playing ? 1 : 0), { playing: Boolean(playing) });
    if (accepted === false) {
      throw new UserFacingError(
  ...
  async closeSourceMonitorClip() {
    ...
    return (await tracePremiereCall("SourceMonitor.closeClip", () => this.api.SourceMonitor.closeClip())) !== false;
  }
  ...
  async projectItemId(projectItem) {
    ...
    const baseItem = tracePremiereCall("ProjectItem.cast", () => this.api.ProjectItem.cast(projectItem));
    return String(await Promise.resolve(tracePremiereCall("ProjectItem.getId", () => baseItem.getId())));
  }
  ...
  async findExactProjectItemsByMediaPath(absolutePath, options = {}) {
    ...
    const project = options.project || await tracePremiereCall("Project.getActiveProject", () => this.api.Project.getActiveProject());
  ...
  async prepareReplaySource(absolutePath, options = {}) {
    ...
    const operation = (async () => {
      ...
      const normalizedMediaPath = normalizeReplayPath(
        await tracePremiereCall("ProjectItem.getMediaFilePath", () => projectItem.getMediaFilePath()),
      );
  ...
  async applyAndVerifyLabel9(projectItem) {
    ...
    if (!project) {
      throw new UserFacingError(
        "The Premiere project closed before Blocky Studios could apply label 9",
        false,
        "LABEL_RECONCILE_FAILED",
      );
    }
    ...
    try {
      verifiedPath = typeof projectItem.getMediaFilePath === "function"
        ? normalizeReplayPath(await tracePremiereCall("ProjectItem.getMediaFilePath", () => projectItem.getMediaFilePath()))
        : "";
      const baseItem = tracePremiereCall("ProjectItem.cast", () => this.api.ProjectItem.cast(projectItem));
      verifiedProjectItemId = String(await Promise.resolve(tracePremiereCall("ProjectItem.getId", () => baseItem.getId())));
    } catch (error) {
  ...
  async collectActiveSequenceTrackItems() {
    ...
    const sequence = project && typeof project.getActiveSequence === "function"
      ? await tracePremiereCall("Project.getActiveSequence", () => project.getActiveSequence())
      : null;
    if (!sequence) {
      ...
      for (let trackIndex = 0; trackIndex < trackCount; trackIndex += 1) {
        try {
          ...
          const trackItems = await Promise.resolve(
            tracePremiereCall("Track.getTrackItems", () => track.getTrackItems(this.api.Constants.TrackItemType.CLIP, false), { trackIndex, kind }),
          );
          let ordinal = 0;
          for (const trackItem of trackItems || []) {
            try {
              const sourceProjectItem = await tracePremiereCall("TrackItem.getProjectItem", () => trackItem.getProjectItem());
              const sourceProjectItemId = await this.projectItemId(sourceProjectItem);
              const startTicks = tickTimeTicks(await tracePremiereCall("TrackItem.getStartTime", () => trackItem.getStartTime()));
              const endTicks = tickTimeTicks(await tracePremiereCall("TrackItem.getEndTime", () => trackItem.getEndTime()));
              const name = typeof trackItem.getName === "function"
                ? String(await tracePremiereCall("TrackItem.getName", () => trackItem.getName()))
                : "";
              const fingerprint = [
                kind,
                trackIndex,
    ...
    const videoTrackCount = Math.max(0, Number(await tracePremiereCall("Sequence.getVideoTrackCount", () => sequence.getVideoTrackCount())) || 0);
    const audioTrackCount = Math.max(0, Number(await tracePremiereCall("Sequence.getAudioTrackCount", () => sequence.getAudioTrackCount())) || 0);
    await collect("video", videoTrackCount, (index) => tracePremiereCall("Sequence.getVideoTrack", () => sequence.getVideoTrack(index), { trackIndex: index }));
    await collect("audio", audioTrackCount, (index) => tracePremiereCall("Sequence.getAudioTrack", () => sequence.getAudioTrack(index), { trackIndex: index }));
 ...
 * proves that the ProjectItem Blocky Studios opened is still current. Blocky Studios must never
...
class PremiereSourceMonitorViewerAdapter {
  ...
  async projectItemId(projectItem) {
    ...
    try {
      return String(await Promise.resolve(tracePremiereCall("ProjectItem.getId", () => projectItem.getId())));
    } catch (error) {
      return "";
    }
  }
  ...
  async projectItemPath(projectItem) {
    ...
    try {
      const clip = tracePremiereCall("ClipProjectItem.cast", () => this.api.ClipProjectItem.cast(projectItem));
      if (!clip || typeof clip.getMediaFilePath !== "function") return "";
      return normalizeReplayPath(await tracePremiereCall("ClipProjectItem.getMediaFilePath", () => clip.getMediaFilePath()));
    } catch (error) {
      return "";
    }
  }
  ...
  async currentOwnership() {
    ...
    let current = null;
    try {
      current = await tracePremiereCall("SourceMonitor.getProjectItem", () => this.api.SourceMonitor.getProjectItem());
    } catch (error) {
  ...
  async requireOwnership() {
    ...
    if (!ownership.owned) {
      this.clearOwnership();
      throw new UserFacingError(
        "Premiere Source Monitor changed outside Blocky Studios. Reopen the replay to restore viewer control.",
        true,
  ...
  async open(replay) {
    ...
    let opened;
    try {
      opened = await tracePremiereCall("SourceMonitor.openFilePath", () => this.api.SourceMonitor.openFilePath(absolutePath));
    } catch (error) {
      ...
      if (/unsupported|codec|decoder|decode|media\s+format|invalid\s+format/i.test(signature)) {
        throw new UserFacingError(
          "Premiere Source Monitor cannot decode this replay codec. Re-export it with a Premiere-supported codec, relink the replacement, and try again. Blocky Studios will not convert media silently.",
          true,
          "SOURCE_MONITOR_CODEC_UNSUPPORTED",
        );
      }
      throw new UserFacingError(
        "Premiere could not open this replay in Source Monitor. Verify the source file still exists; if it does, re-export it with a Premiere-supported codec and relink it. Blocky Studios will not convert media silently.",
        true,
    ...
    for (let attempt = 0; attempt < 30; attempt += 1) {
      try {
        projectItem = await tracePremiereCall("SourceMonitor.getProjectItem", () => this.api.SourceMonitor.getProjectItem());
      } catch (error) {
        projectItem = null;
      }
    ...
    if (!projectItemId || pathKey(mediaPath) !== expectedPathKey) {
      ...
      // editor. With no exact-path identity proof Blocky Studios must not close it.
      this.clearOwnership();
      throw new UserFacingError(
  ...
  async getPosition() {
    await this.requireOwnership();
    const position = await tracePremiereCall("SourceMonitor.getPosition", () => this.api.SourceMonitor.getPosition());
  ...
  async seek(seconds) {
    ...
    const time = tracePremiereCall("TickTime.createWithSeconds", () => this.api.TickTime.createWithSeconds(value));
    const accepted = await tracePremiereCall("SourceMonitor.setPosition", () => this.api.SourceMonitor.setPosition(time));
    if (accepted === false) {
      throw new UserFacingError(
  ...
  async close(ownershipToken = this.ownershipToken) {
    ...
    try {
      await tracePremiereCall("SourceMonitor.play", () => this.api.SourceMonitor.play(0), { playing: false });
    } catch (error) {
    ...
    const closed = await tracePremiereCall("SourceMonitor.closeClip", () => this.api.SourceMonitor.closeClip());
    if (closed === false) {
      throw new UserFacingError(
        "Premiere could not release the Blocky Studios Source Monitor replay.",
        true,
...
class SmoothWheelScroller {
  ...
  start() {
    ...
    this.started = true;
    // Deliberately no wheel listener: Premiere/UXP owns wheel propagation and
    // boundary behavior for this native scroll container.
  }
...
class ReplayGridView {
  constructor(elements, callbacks) {
    ...
    this.onThumbnailUsed = typeof callbacks.onThumbnailUsed === "function"
      ...
      : () => undefined;
    this.onThumbnailNeeded = typeof callbacks.onThumbnailNeeded === "function"
      ? callbacks.onThumbnailNeeded
      : () => undefined;
    ...
    this.getCollectionById = typeof callbacks.getCollectionById === "function"
      ? callbacks.getCollectionById
      : () => null;
  ...
  renderCards(replays, options = {}) {
    ...
    const nextActiveCollectionId = String(options.activeCollectionId || "");
    const manualOrderChanged = nextManualOrderCollectionId !== this.manualOrderCollectionId ||
      nextActiveCollectionId !== this.activeCollectionId;
    this.manualOrderCollectionId = nextManualOrderCollectionId;
    this.activeCollectionId = nextActiveCollectionId;
  ...
  estimateCardRowHeight(columns = this.effectiveColumns()) {
    ...
    return Math.max(210, cardWidth * 9 / 16 + 96 + gap);
  }
  ...
  renderVirtualWindow() {
    ...
    for (let index = range.start; index < range.end; index += 1) {
      ...
      for (let columnIndex = 0; columnIndex < row.ids.length; columnIndex += 1) {
        ...
        if (!replay) continue;
        const key = String(id);
        const signature = `${replayCardSignature(replay)}|manual:${this.manualOrderCollectionId}|collection:${this.activeCollectionId}`;
  ...
  createCard(replay) {
    ...
    card.setAttribute(
      "aria-label",
      ready
        ? `${replay.title}. Double-click or press Enter to play in Blocky Studios Viewer. Drag to place it on the Premiere timeline.`
        : `${replay.title}. ${statusLabel(replay)}.`,
    );
    card.title = ready
      ? "Double-click to play · Drag to the Premiere timeline"
      : statusLabel(replay);
    ...
    if (this.manualOrderCollectionId) {
      ...
      reorderHandle.className = "oracle-icon-button oracle-icon-button--compact replay-reorder-handle";
      ...
      const reorderIcon = this.document.createElement("img");
      reorderIcon.className = "bs-icon";
      reorderIcon.setAttribute("src", "assets/icons/reorder.png");
      reorderIcon.setAttribute("alt", "");
      reorderIcon.setAttribute("aria-hidden", "true");
      reorderHandle.appendChild(reorderIcon);
      thumbnail.appendChild(reorderHandle);
    }
    ...
    if (thumbnailSource) {
      ...
      if (replay.missingState === "available") this.onThumbnailNeeded(replay);
    }
    ...
    const metadataRow = this.document.createElement("div");
    metadataRow.className = "replay-card__metadata";
    const collectionId = this.activeCollectionId || (Array.isArray(replay.collectionIds) ? replay.collectionIds[0] : "");
    const collection = collectionId ? this.getCollectionById(collectionId) : null;
    if (collection) metadataRow.appendChild(createReplayCollectionChip(collection));
    const tags = Array.isArray(replay.tags) ? replay.tags : [];
    for (const tag of tags.slice(0, 2)) metadataRow.appendChild(createReplayTagChip(tag));
    if (tags.length > 2) metadataRow.appendChild(createReplayTagOverflow(tags.length - 2));
    if (replay.missingState === "missing" || replay.missingState === "unknown") {
      metadataRow.appendChild(createReplayIndicator("Source unavailable", "missing"));
    }
    ...
    resolution.textContent = formatReplaySpecs(replay);
    ...
    if (metadataRow.children.length) copy.appendChild(metadataRow);
  ...
  onReplayPointerDown(event) {
    ...
    if (typeof cardElement.setPointerCapture === "function") {
      try {
        ...
        if (NATIVE_DRAG_DEBUG) {
          console.debug("Blocky Studios could not capture the replay drag pointer.", error);
        }
      }
    }
  }
  ...
  finishReplayGesture(dragState, reason) {
    ...
    if (NATIVE_DRAG_DEBUG) {
      console.debug(`[Blocky Studios Native Drag][Gesture End] ${reason}`, {
  ...
  async beginNativeReplayDrag(dragState, thresholdEvent) {
    ...
    const pollNativeSnapshot = () => {
      ...
      try {
        const snapshot = traceNativeCall("getNativeDragSnapshot", () => this.nativeAddon.getNativeDragSnapshot());
        const stage = String((snapshot && snapshot.stage) || "UNKNOWN");
        if (stage !== lastSnapshotStage) {
          lastSnapshotStage = stage;
          console.log("[Blocky Studios Native Drag][Snapshot]", {
            gestureId: dragState.gestureId,
            replayId: dragState.replayId,
            ...snapshot,
          });
          console.log(
            "[Blocky Studios Native Drag][Snapshot JSON]",
            JSON.stringify({
              gestureId: dragState.gestureId,
              replayId: dragState.replayId,
              ...snapshot,
        ...
        console.error("[Blocky Studios Native Drag][Snapshot Failed]", error);
        return null;
      }
    };
    try {
      if (
        ...
        result = this.nativeAddonDiagnostic || nativeDragError(
          "ADDON_RETURNED_NULL",
          "the Blocky Studios native drag addon is unavailable without a load diagnostic.",
        );
      } else {
        ...
        const dragPromise = traceNativeCall(
          "startNativeFileDrag",
          () => this.nativeAddon.startNativeFileDrag(nativeDragPath),
          { gestureId: dragState.gestureId, replayId: dragState.replayId },
        );
        ...
        if (NATIVE_DRAG_DEBUG) {
          ...
          try {
            selfTest = typeof this.nativeAddon.nativeSelfTest === "function"
              ? traceNativeCall("nativeSelfTest", () => this.nativeAddon.nativeSelfTest())
              : null;
          } catch (error) {
            selfTest = { ok: false, errorMessage: String((error && error.message) || error) };
          }
          console.log("[Blocky Studios Native Drag][Invocation]", {
            gestureId: dragState.gestureId,
            replayId: dragState.replayId,
    ...
    if (this.nativeAddon && typeof this.nativeAddon.getNativeDragSnapshot === "function") {
      try {
        completionSnapshot = traceNativeCall("getNativeDragSnapshot", () => this.nativeAddon.getNativeDragSnapshot());
      } catch (error) {
    ...
    if (NATIVE_DRAG_DEBUG) {
      console.log("[Blocky Studios Native Drag][Result]", {
        gestureId: dragState.gestureId,
        replayId: dragState.replayId,
        ...result,
      });
  ...
  handleNativeDragResult(replay, result, cardElement, canonicalPath) {
    ...
    if (NATIVE_DRAG_DEBUG) {
      console.debug("Blocky Studios native replay drag result", nativeResult);
    }
  ...
  releaseReplayPointerCapture(dragState) {
    ...
    try {
      ...
      if (NATIVE_DRAG_DEBUG) {
        console.debug("Blocky Studios could not release the replay drag pointer.", error);
      }
    }
  }
  ...
  onContextMenuClick(event) {
    ...
    if (!replay) return;
    if (action === "play") this.onOpen(replay);
  ...
  logGestureEvent(eventType, event, state, extra = {}) {
    ...
    console.log("[Blocky Studios Native Drag][Event]", record);
  }
...
class GridScaleControl {
  constructor(input, grid, countLabel, onCommittedChange = null, onLayoutChanged = null) {
    ...
    this.handleResize = (entries) => {
      const entry = Array.isArray(entries) ? entries[0] : null;
      const observedWidth = entry && entry.contentRect
        ? Number(entry.contentRect.width)
        : Number(this.grid && this.grid.clientWidth);
  ...
  applyLayout() {
    ...
    this.grid.style.setProperty("--replay-card-flex-basis", `${cardBasis}px`);
    this.grid.style.setProperty("--replay-card-basis", `${cardBasis}px`);
    if (this.countLabel) {
...
class OraclePanelController {
  constructor() {
    ...
    this.elements = {
      ...
      replayCollectionStrip: document.getElementById("replayCollectionStrip"),
      replayArchiveAttentionCount: document.getElementById("replayArchiveAttentionCount"),
      ...
      replayViewerMedia: document.getElementById("replayViewerMedia"),
      replayViewerPlayPause: document.getElementById("replayViewerPlayPause"),
      replayViewerStop: document.getElementById("replayViewerStop"),
      ...
      replayViewerFrame: document.getElementById("replayViewerFrame"),
      ...
      replayViewerRateMenu: document.getElementById("replayViewerRateMenu"),
      replayViewerLoop: document.getElementById("replayViewerLoop"),
      replayViewerProgress: document.getElementById("replayViewerProgress"),
      replayViewerRetry: document.getElementById("replayViewerRetry"),
      ...
      replayViewerOpenSource: document.getElementById("replayViewerOpenSource"),
      ...
      curvesSettingsToggle: document.getElementById("curvesSettingsToggle"),
      curvesSettingsPanel: document.getElementById("curvesSettingsPanel"),
      curvesSettingsClose: document.getElementById("curvesSettingsClose"),
      curvesInspector: document.getElementById("curvesInspector"),
      curvesResizeHandle: document.getElementById("curvesResizeHandle"),
      curvesDetectedProperties: document.getElementById("curvesDetectedProperties"),
      ...
      curvesGraphTitle: document.getElementById("curvesGraphTitle"),
      ...
      curvesEndpointStart: document.getElementById("curvesEndpointStart"),
      curvesEndpointEnd: document.getElementById("curvesEndpointEnd"),
      ...
      curvesZoomLabel: document.getElementById("curvesZoomLabel"),
      curvesGridSize: document.getElementById("curvesGridSize"),
      curvesGrid4: document.getElementById("curvesGrid4"),
      curvesGrid8: document.getElementById("curvesGrid8"),
      curvesGrid16: document.getElementById("curvesGrid16"),
      curvesPresetViewList: document.getElementById("curvesPresetViewList"),
      curvesPresetViewGrid: document.getElementById("curvesPresetViewGrid"),
      curvesInterpolationBezier: document.getElementById("curvesInterpolationBezier"),
      curvesInterpolationLinear: document.getElementById("curvesInterpolationLinear"),
      curvesInterpolationHold: document.getElementById("curvesInterpolationHold"),
      ...
      (columns) => {
        if (this.theme && typeof this.theme.commitExternal === "function") {
          try {
            ...
            this.showToast("Blocky Studios could not persist the grid density.", "error");
          }
        }
      },
      () => {
    ...
    const replayMediaApi = oracleWindow.OracleReplayMedia;
    ...
    if (!preferencesApi || typeof preferencesApi.OraclePreferencesController !== "function") {
      throw new Error("Blocky Studios Preferences module did not load.");
    }
    if (!shellApi || typeof shellApi.OracleShellController !== "function") {
      throw new Error("Blocky Studios shell module did not load.");
    }
    if (!replayLibraryApi || typeof replayLibraryApi.ReplayLibraryStore !== "function") {
      throw new Error("Blocky Studios v3 replay library module did not load.");
    }
    if (!replayMediaApi || typeof replayMediaApi.ReplayMediaPipeline !== "function") {
      throw new Error("Blocky Studios replay media module did not load.");
    }
    if (!replayWorkspaceApi || typeof replayWorkspaceApi.ReplayWorkspaceController !== "function") {
      throw new Error("Blocky Studios replay workspace module did not load.");
    }
    if (!replayOrganizationApi) {
      throw new Error("Blocky Studios replay organization module did not load.");
    }
    if (!replayLifecycleUiApi || typeof replayLifecycleUiApi.ReplayLifecycleDialogController !== "function") {
      throw new Error("Blocky Studios replay lifecycle UI module did not load.");
    }
    if (!replayViewerApi || typeof replayViewerApi.ReplayViewerController !== "function") {
      throw new Error("Blocky Studios replay viewer module did not load.");
    }
    if (!curveMathApi || typeof curveMathApi.createBakedCurveSamples !== "function") {
      throw new Error("Blocky Studios curve math module did not load.");
    }
    if (!curvePresetApi || typeof curvePresetApi.normalizePresetLibrary !== "function") {
      throw new Error("Blocky Studios curve preset module did not load.");
    }
    if (!curvesAdapterApi || typeof curvesAdapterApi.PremiereCurvesAdapter !== "function") {
      throw new Error("Blocky Studios Premiere Curves adapter did not load.");
    }
    if (!curvesWorkspaceApi || typeof curvesWorkspaceApi.CurvesWorkspaceController !== "function") {
      throw new Error("Blocky Studios Curves workspace module did not load.");
    }
    if (!effectIndexApi || typeof effectIndexApi.createEffectIndex !== "function") {
      throw new Error("Blocky Studios supported effect index module did not load.");
    }
    if (!effectsAdapterApi || typeof effectsAdapterApi.PremiereQuickApplyAdapter !== "function") {
      throw new Error("Blocky Studios Premiere Quick Apply adapter did not load.");
    }
    if (!quickApplyDomainApi || typeof quickApplyDomainApi.QuickApplyDomain !== "function") {
      throw new Error("Blocky Studios Quick Apply domain module did not load.");
    }
    if (!quickApplyWorkspaceApi || typeof quickApplyWorkspaceApi.QuickApplyWorkspaceController !== "function") {
      throw new Error("Blocky Studios Quick Apply workspace module did not load.");
    }
    if (
      !runtimeRegistryApi ||
      ...
      throw new Error("Blocky Studios multi-panel runtime registry module did not load.");
    }
    this.replayLibraryApi = replayLibraryApi;
    this.replayMediaApi = replayMediaApi;
    ...
    this.mainRoot = resolveOracleMainPanelShell(document) || document.body;
    this.uiRuntime = oracleWindow.oracleUiRuntime || null;
    ...
    this.replayMedia = new replayMediaApi.ReplayMediaPipeline({
      nativeAddon: nativeDragAddon,
      thumbnailCache: this.thumbnailCache,
      fs: uxpFs,
      getCacheDirectory: () => oracleDataFolderNativePath(),
      getCacheLimitMb: () => Number(
    ...
    this.internalViewerAdapter = new replayViewerApi.HtmlVideoReplayAdapter(
      this.elements.replayViewerMedia,
      { mediaPipeline: this.replayMedia },
    );
    this.viewer = new replayViewerApi.ReplayViewerController({
      ...
      media: this.elements.replayViewerMedia,
      playPause: this.elements.replayViewerPlayPause,
      stop: this.elements.replayViewerStop,
      ...
      frame: this.elements.replayViewerFrame,
      ...
      speedMenu: this.elements.replayViewerRateMenu,
      loop: this.elements.replayViewerLoop,
      progress: this.elements.replayViewerProgress,
      retry: this.elements.replayViewerRetry,
      close: this.elements.replayViewerClose,
      error: this.elements.replayViewerError,
      support: this.elements.replayViewerSupport,
      openSource: this.elements.replayViewerOpenSource,
    }, {
      adapter: this.internalViewerAdapter,
      onOpenSourceMonitor: (replay) => this.openInSourceMonitor(replay),
      onUsage: (replayOrId) => {
    ...
    this.view = new ReplayGridView(this.elements, {
      ...
      onThumbnailNeeded: (replay) => this.ensureReplayThumbnail(replay && replay.id),
      getReplayById: (id) => this.store && this.store.getById(id),
      getCollectionById: (id) => this.store && this.store.state && this.store.state.collectionsById[id],
    ...
    this.curvesWorkspace = new curvesWorkspaceApi.CurvesWorkspaceController(this.elements, {
      ...
      onAnnounce: (message) => this.announce(message),
      viewportService: this.uiRuntime,
      viewportRoot: this.mainRoot,
      viewportPanelId: "oraclePanel",
      fontReady: document.fonts && document.fonts.ready,
      document,
    });
  ...
  applyMainPanelRoute(route, reason = "route-change") {
    ...
    if (this.curvesWorkspace) {
      if (typeof this.curvesWorkspace.setLifecycle === "function") {
        this.curvesWorkspace.setLifecycle({ visible: curvesActive, active: curvesActive });
      } else {
    ...
    if (this.quickApplyWorkspace) {
      if (typeof this.quickApplyWorkspace.setLifecycle === "function") {
        this.quickApplyWorkspace.setLifecycle({ visible: quickApplyActive, active: quickApplyActive });
      } else {
  ...
  setMainPanelVisible(value) {
    ...
    if (this.uiRuntime && typeof this.uiRuntime.setVisible === "function") {
      this.uiRuntime.setVisible(this.mainRoot, next);
    }
  ...
  async start() {
    ...
    if (recovered.source !== "primary") {
      try {
        ...
        if (!this.destroyed) {
          this.showToast("Blocky Studios restored replay metadata but could not repair the primary state file.", "error");
        }
      }
    }
    ...
    return {
      ...
      saveAs: async (payload) => {
        const name = await this.chooseCurvePresetName({
          ...
          initialValue: payload.preset ? `${payload.preset.name} Copy` : "Blocky Studios Curve",
          applyLabel: "Save As",
        }, dialog);
        if (!name) return { ok: true, cancelled: true, library: payload.library };
        return this.commitCurvePresetResult(
      ...
      duplicate: async (payload) => {
        ...
        if (!name) return { ok: true, cancelled: true, library: payload.library };
        return this.commitCurvePresetResult(
          this.curvePresetApi.copyPresetToUser(payload.library, payload.preset.id, { name }),
          "duplicate",
        );
      },
      ...
      folderCreate: async (payload) => {
        const name = await this.chooseCurvePresetName({
          ...
          message: "Create a folder for organizing Blocky Studios user curve presets.",
      ...
      folderRename: async (payload) => {
        ...
        const name = await this.chooseCurvePresetName({
          ...
          message: "Rename this Blocky Studios curve preset folder.",
      ...
      import: async (payload) => {
        ...
        if (String(entry.name || "").toLowerCase().endsWith(".prfpset")) {
          throw new Error("Premiere .prfpset files are not Blocky Studios curve libraries.");
        }
        let rawSize;
        if (typeof entry.getMetadata === "function") {
          ...
          try {
            ...
            throw new Error("Blocky Studios could not verify the curve preset file size before reading it.");
          }
      ...
      export: async (payload) => {
        ...
        const exported = this.curvePresetApi.exportPresetLibrary(payload.library, {
          filename: "Blocky-Studios-Curve-Presets.json",
        });
  ...
  confirmCurvePresetAction(request = {}, dialog = this.curvesPresetDialog) {
    ...
    return dialog.confirm({
      ...
      message: action === "delete"
        ? `Delete ${presetName}? This removes only Blocky Studios preset metadata and never touches Premiere media.`
  ...
  requestQuickApplyRecipeName(request = {}, dialog = this.curvesPresetDialog) {
    ...
    return dialog.requestName({
      kicker: "Blocky Studios Recipes",
      nameLabel: "Blocky Studios Recipe",
      title: String(request.title || (mode === "rename" ? "Rename Blocky Studios Recipe" : "Name Blocky Studios Recipe")),
      message: mode === "duplicate"
        ? "Choose a distinct name for the duplicated ordered effect stack."
        : mode === "rename"
          ? "Rename only Blocky Studios recipe metadata; installed Premiere effects are unchanged."
          : "Choose a name for this ordered, verified Premiere effect stack.",
      initialValue: String(request.value || "New Blocky Studios Recipe"),
  ...
  confirmQuickApplyRecipeAction(request = {}, dialog = this.curvesPresetDialog) {
    const recipeName = String(request.recipe && request.recipe.name || "this Blocky Studios Recipe");
    return dialog.confirm({
      kicker: "Blocky Studios Recipes",
      nameLabel: "Blocky Studios Recipe",
      title: String(request.title || "Delete Blocky Studios Recipe?"),
      message: String(request.message || `Delete ${recipeName}? This removes only Blocky Studios recipe metadata and never changes Premiere media.`),
      applyLabel: "Delete",
    });
  }
  ...
  async importQuickApplyRecipes(_request = {}) {
    ...
    const picker = storage && storage.localFileSystem;
    if (!picker || typeof picker.getFileForOpening !== "function") {
      throw new Error("Blocky Studios Recipe import needs the UXP JSON file picker.");
    }
    const entry = await picker.getFileForOpening({ types: ["json"], allowMultiple: false });
    ...
    if (!Number.isSafeInteger(importLimit) || importLimit <= 0 || typeof domainApi.utf8ByteLength !== "function") {
      throw new Error("Blocky Studios Recipe import limits are unavailable in this runtime.");
    }
    let rawSize;
    if (typeof entry.getMetadata === "function") {
      let metadata;
      try {
        ...
        throw new Error("Blocky Studios could not verify the selected JSON file size before reading it.");
      }
      rawSize = metadata && metadata.size;
    } else rawSize = entry.size;
    ...
    if (entrySize > importLimit) throw new Error("Blocky Studios Recipe JSON exceeds the 2 MB import limit.");
    const text = String(await entry.read({ format: storage.formats.utf8 }) || "");
    if (domainApi.utf8ByteLength(text) > importLimit) {
      throw new Error("Blocky Studios Recipe JSON exceeds the 2 MB import limit.");
    }
    return {
      text,
      filename: String(entry.name || "Blocky-Studios-Recipes.json").slice(0, 260),
    };
  }
  ...
  async exportQuickApplyRecipes(request = {}) {
    ...
    const picker = storage && storage.localFileSystem;
    if (!picker || typeof picker.getFileForSaving !== "function") {
      throw new Error("Blocky Studios Recipe export needs the UXP JSON file picker.");
    }
    const suggestedName = String(request.suggestedName || "Blocky-Studios-Recipes.json").slice(0, 260);
  ...
  refreshReplayWorkspace(change = {}) {
    ...
    const collections = Object.values(state.collectionsById || {})
      ...
      .slice(0, 512)
      .map((collection) => ({ id: collection.id, name: collection.name, color: collection.color || "" }));
    ...
    if (query.collectionId && !collections.some((entry) => entry.id === query.collectionId)) {
      ...
      if (selected) collections.unshift({ id: selected.id, name: selected.name, color: selected.color || "" });
    }
    this.workspace.setFacets({ collections, tags, roots });
    this.workspace.setArchiveAttentionCount(records.filter((record) => (
      record.missingState === "missing" || record.missingState === "unknown"
    )).length);
    this.workspace.setResultCount(items.length, records.length);
    this.view.render(items, {
      totalCount: records.length,
      resetScroll: change && change.resetScroll === true,
      activeCollectionId: query.collectionId || "",
  ...
  canApplyReplayChangeInPlace(change) {
    ...
    for (const field of fields) {
      ...
      if (field === "missingState") return false;
  ...
  requireOrganizationPlan(plan) {
    ...
    const message = String((plan && plan.message) || "Blocky Studios could not apply this replay-library action.");
  ...
  updateReplaySelectionUi(ids, replays, elements = this.elements) {
    ...
    const relink = /** @type {HTMLElement | null} */ (
      elements.replayBatchBar.querySelector('[data-replay-batch-action="relink"]')
    );
    if (relink) relink.hidden = count === 0 || !selected.every(replayRequiresRelink);
  }
  ...
  getReplayContextActionState(action, replay, selected, viewer = this.viewer) {
    ...
    const replayRequiresRelink = (value) => Boolean(
      value && (value.missingState === "missing" || value.missingState === "unknown"),
    );
    if (action === "play") return { disabled: !single, reason: single ? "" : "Playback applies to one replay at a time." };
    if (action === "source-monitor") {
      return {
        disabled: !single || !this.viewerAdapter || !this.viewerAdapter.isAvailable(),
        reason: !single
          ? "Source Monitor applies to one replay at a time."
          : !this.viewerAdapter || !this.viewerAdapter.isAvailable()
            ? "Premiere Source Monitor is unavailable."
            : "",
      };
    }
    if (["rename-display", "reveal"].includes(action) && !single) {
    ...
    if (action === "relink") {
      return {
        hidden: !replayRequiresRelink(replay),
        disabled: !nativeAvailable || typeof nativeDragAddon.inspectReplayFileIdentity !== "function",
        reason: nativeAvailable && typeof nativeDragAddon.inspectReplayFileIdentity === "function"
          ? ""
          : "The file-identity service required for relinking is unavailable.",
      };
    }
    return {};
  }
  ...
  restartNativeDirectoryWatch(extraPaths = []) {
    ...
    if (this.nativeLifecycleStarted) traceNativeCall("stopDirectoryWatch", () => nativeDragAddon.stopDirectoryWatch());
    ...
    const result = this.nativeLifecycleResult(
      traceNativeCall("startDirectoryWatch", () => nativeDragAddon.startDirectoryWatch(this.nativeWatchRoots), { rootCount: this.nativeWatchRoots.length }),
      "DIRECTORY_WATCH_START_FAILED",
      "Blocky Studios could not start the bounded replay directory watcher.",
    );
  ...
  async ensureNativeReplayRegistration(replayOrId, options = {}) {
    ...
    const task = (async () => {
      ...
      const inspection = this.nativeLifecycleResult(
        traceNativeCall("inspectReplayFileIdentity", () => nativeDragAddon.inspectReplayFileIdentity(record.canonicalPath), { replayId: id }),
        "FILE_IDENTITY_FAILED",
        "Blocky Studios could not inspect the replay source identity.",
      );
      ...
      const registration = this.nativeLifecycleResult(
        traceNativeCall("registerKnownReplayFile", () => nativeDragAddon.registerKnownReplayFile(id, inspection.path, inspection.identityKey), { replayId: id }),
        "FILE_REGISTRATION_FAILED",
        "Blocky Studios could not register the replay for safe file operations.",
      );
  ...
  async replayFileCandidate(absolutePath) {
    ...
    const inspection = this.nativeLifecycleResult(
      traceNativeCall("inspectReplayFileIdentity", () => nativeDragAddon.inspectReplayFileIdentity(absolutePath)),
          ...
          if (requiresConfirmation) {
            ambiguityReason = scored.ambiguous
              ...
              : match.sameVolume === false
                ? scored.honestBoundary
                : "The match score is below the Blocky Studios automatic-confidence threshold.";
          }
        }
      }
      if (!chosen) continue;
  ...
  async revealReplayInExplorer(replay, showToast = (message, kind) => this.showToast(message, kind)) {
    ...
    const result = this.nativeLifecycleResult(
      traceNativeCall("revealFileInExplorer", () => nativeDragAddon.revealFileInExplorer(replay.id), { replayId: replay.id }),
      "REVEAL_FAILED",
  ...
  pauseNativeDirectoryWatchForMutation() {
    ...
    if (wasRunning && nativeDragAddon && typeof nativeDragAddon.stopDirectoryWatch === "function") {
      traceNativeCall("stopDirectoryWatch", () => nativeDragAddon.stopDirectoryWatch());
      this.nativeLifecycleStarted = false;
    }
    return () => {
  ...
  async renameReplaySource(replay, filename, context = {}) {
    ...
    try {
      nativeResult = this.nativeLifecycleResult(
        traceNativeCall("renameKnownReplayFile", () => nativeDragAddon.renameKnownReplayFile(replay.id, plan.targetPath), { replayId: replay.id }),
    ...
    try {
      ...
      const rollback = traceNativeCall("renameKnownReplayFile", () => nativeDragAddon.renameKnownReplayFile(replay.id, plan.sourcePath), { replayId: replay.id, rollback: true });
  ...
  async commitReplayRelinks(selection, context = {}) {
    ...
    for (const assignment of assignments) {
      ...
      try {
        ...
        const registration = this.nativeLifecycleResult(
          traceNativeCall("registerKnownReplayFile", () => nativeDragAddon.registerKnownReplayFile(replayId, candidate.canonicalPath, candidate.fileIdentity.key), { replayId }),
          "RELINK_REGISTRATION_FAILED",
          "Blocky Studios could not register the replacement for safe lifecycle tracking.",
        );
        ...
        if (nativeRegistrationChanged && !domainCommitted && nativeDragAddon) {
          if (typeof nativeDragAddon.unregisterKnownReplayFile === "function") {
            traceNativeCall("unregisterKnownReplayFile", () => nativeDragAddon.unregisterKnownReplayFile(replayId), { replayId });
          }
          if (previous && previous.missingState !== "missing" && typeof nativeDragAddon.registerKnownReplayFile === "function") {
            try {
              traceNativeCall("registerKnownReplayFile", () => nativeDragAddon.registerKnownReplayFile(
                replayId,
                previous.canonicalPath,
                this.nativeIdentityKey(previous),
              ), { replayId, rollback: true });
            } catch (rollbackError) {
              // Leaving the ID unregistered is safer than retaining a registry
              // entry for media Blocky Studios did not commit.
            }
          }
        }
  ...
  async applyReplayDelete(mode, payload, context) {
    ...
    if (mode === "remove-metadata") {
      ...
      for (const id of ids) {
        if (nativeDragAddon && typeof nativeDragAddon.unregisterKnownReplayFile === "function") {
          traceNativeCall("unregisterKnownReplayFile", () => nativeDragAddon.unregisterKnownReplayFile(id), { replayId: id });
        }
    ...
    if (readyIds.length) {
      ...
      if (readyIds.length) {
        const resumeWatcher = this.pauseNativeDirectoryWatchForMutation();
        try {
          const operation = /** @type {any} */ (traceNativeCall("recycleKnownFiles", () => nativeDragAddon.recycleKnownFiles(readyIds), { replayCount: readyIds.length }));
    ...
    for (const item of operationResult.items) {
      if (item.status === "success" && typeof nativeDragAddon.unregisterKnownReplayFile === "function") {
        traceNativeCall("unregisterKnownReplayFile", () => nativeDragAddon.unregisterKnownReplayFile(item.replayId), { replayId: item.replayId });
      } else {
  ...
  cancelReplayLifecycleOperation(mode, context) {
    ...
    const result = traceNativeCall("cancelFileOperation", () => nativeDragAddon.cancelFileOperation(requestId), { requestId });
    const showToast = context && typeof context.showToast === "function"
      ? context.showToast
      : (message, kind) => this.showToast(message, kind);
  ...
  pollNativeDirectoryWatch() {
    ...
    let events;
    try {
      events = traceNativeCall("pollDirectoryWatchEvents", () => nativeDragAddon.pollDirectoryWatchEvents(256), { maximumEvents: 256 });
    } catch (error) {
      return;
    }
      ...
      this.trackNativeWatcherTask((async () => {
        ...
        try {
          ...
          try {
            ...
            this.showToast("Blocky Studios could not release the missing replay from every Source Monitor viewer.", "error");
          }
          if (this.metadataMutationActive || this.destroyed) return;
  ...
  async reconcileNativeWatcherRename(event) {
    ...
    try {
      ...
      const registration = this.nativeLifecycleResult(
        traceNativeCall("registerKnownReplayFile", () => nativeDragAddon.registerKnownReplayFile(record.id, candidate.canonicalPath, candidate.fileIdentity.key), { replayId: record.id }),
        "WATCHER_RECONCILIATION_FAILED",
        "Blocky Studios could not reconcile the renamed source with its native file registry.",
      );
      nativeRegistrationChanged = true;
      const committed = this.requireOrganizationPlan(
        this.replayOrganizationApi.commitGuardedReplayMutation(this.store.state, plan, {
      ...
      if (references.length) {
        this.showToast("The source moved outside Blocky Studios. Premiere still references the old path; use Premiere's supported relink UI.", "error");
      }
    } catch (error) {
      if (nativeRegistrationChanged && !domainCommitted && nativeDragAddon) {
        if (typeof nativeDragAddon.unregisterKnownReplayFile === "function") {
          traceNativeCall("unregisterKnownReplayFile", () => nativeDragAddon.unregisterKnownReplayFile(record.id), { replayId: record.id });
        }
  ...
  reconcileBridgeReplayPath(replayId, oldPath, newPath, identityKey) {
    ...
    if (!queued) {
      ...
      const message = failureCode === "OUTBOX_NOT_DURABLE"
        ? "Replay-path delivery could not be saved for panel restart. Keep this panel open until Blocky Studios acknowledges the move, then verify the replay."
        : failureCode === "OUTBOX_FULL"
          ...
          : "Blocky Studios rejected unsafe replay-path reconciliation data. Verify the moved replay before continuing.";
      this.showToast(
        message,
        "error",
      );
  ...
  async clearThumbnailCache() {
    ...
    const previews = this.replayMedia && typeof this.replayMedia.clearManagedPreviews === "function"
      ? await this.replayMedia.clearManagedPreviews()
      : { removed: 0 };
    await this.thumbnailCache.clear();
    this.store.clearThumbnailMetadata();
    this.persistReplayState();
    this.showToast("Replay thumbnail and managed viewer caches cleared.", "success");
    return { ...this.thumbnailCache.usage(), managedPreviewsRemoved: Number(previews.removed) || 0 };
  }
  ...
  async importReplayMetadata(payload) {
    ...
    if (!normalized || normalized.ok !== true) {
      ...
      const message = String(normalized && normalized.message || "Blocky Studios rejected the replay metadata import.");
      throw new Error(`[${code}] ${message}`);
    }
    ...
    const confirmed = await this.curvesPresetDialog.confirm({
      kicker: "Blocky Studios Metadata",
      title: "Replace current Blocky Studios metadata?",
  ...
  async resetReplayMetadata() {
    ...
    const confirmed = await this.curvesPresetDialog.confirm({
      kicker: "Blocky Studios Metadata",
      title: "Reset stored Blocky Studios metadata?",
      message: `Remove ${previousCounts.replayCount} replay record${previousCounts.replayCount === 1 ? "" : "s"}, ${previousCounts.collectionCount} collection${previousCounts.collectionCount === 1 ? "" : "s"}, ${previousCounts.curvePresetCount} Curves preset${previousCounts.curvePresetCount === 1 ? "" : "s"}, and ${previousCounts.recipeCount} Quick Apply recipe${previousCounts.recipeCount === 1 ? "" : "s"} from the Blocky Studios metadata store? This metadata action does not delete source media; thumbnail cache files remain governed by the cache limit.`,
  ...
  async replaceReplayMetadataState(nextState, reason, counts) {
    ...
    try {
      ...
      if (this.destroyed) throw new Error("Blocky Studios closed before the metadata transaction could begin.");
      ...
      if (nativeDragAddon && typeof nativeDragAddon.unregisterKnownReplayFile === "function") {
        for (const id of previousIds) {
          this.nativeLifecycleResult(
            ...
            "Blocky Studios could not release a native replay registration before replacing metadata.",
          );
  ...
  async handleBridgeMessage(message) {
    ...
    if (eventName === "REPLAY_PATH_RECONCILED_NACK") {
      const reason = typeof message.message === "string" && message.message.trim()
        ...
        : "Blocky Studios could not reconcile the replay path.";
    ...
    if (
      ...
      if (replay) {
        this.persistReplayState();
        this.announce(`${replay.title} received from Blocky Studios.`);
    ...
    if (eventName === "BRIDGE_ERROR") {
      this.showToast(message.message || "The Blocky Studios bridge rejected a message.", "error");
    }
  }
  ...
  async persistOracleState(options = {}) {
    ...
    try {
      ...
      if (!this.destroyed) {
        ...
        this.showToast("Blocky Studios could not persist replay metadata safely.", "error");
      }
  ...
  ensureReplayThumbnail(replayId) {
    const id = String(replayId || "");
    if (!id || this.destroyed || !this.store) return false;
    const record = this.store.getRecord(id);
    if (
      !record || record.thumbnailCacheKey || record.missingState !== "available" ||
      (this.processingTasks.get(id) && this.processingTasks.get(id).size)
    ) {
      return false;
    }
    this.scheduleReplayProcessing(id, "visible-thumbnail");
    return true;
  }
  ...
  scheduleReplayProcessing(replayId, origin) {
    ...
    const processingTask = this.metadataQueue.submit(async () => {
      ...
      if (!thumbnailCacheKey && missingState === "available" && this.replayMedia && this.replayMedia.isAvailable()) {
        try {
          if (!matchesScheduledRecord() || this.processingGenerations.get(replayId) !== generation) return;
          const legacy = record.legacy || {};
          const prepared = await this.replayMedia.prepare({ ...record, fileSize, modifiedAt }, {
          ...
          if (!matchesScheduledRecord() || this.processingGenerations.get(replayId) !== generation) return;
          if (!prepared.thumbnail || !prepared.thumbnail.key) {
            throw new Error("Native replay thumbnail preparation returned no adopted cache entry.");
          }
          thumbnailCacheKey = prepared.thumbnail.key;
          thumbnailStatus = "ready";
          thumbnailError = "";
        } catch (error) {
          thumbnailStatus = "error";
          ...
          if (typeof recordOracleDiagnostic === "function") {
            recordOracleDiagnostic("warn", error && error.code || "REPLAY_THUMBNAIL_PREPARATION_FAILED", {
              replayIdPresent: Boolean(replayId),
              origin: String(origin || "unknown"),
              message: typeof oracleErrorMessage === "function"
                ? oracleErrorMessage(error, "Native replay thumbnail preparation failed.")
                : "Native replay thumbnail preparation failed.",
            });
          }
        }
      }
  ...
  scheduleLabelReconciliation(absolutePath, feedback = {}) {
    ...
    this.gateway.scheduleLabelReconciliation(absolutePath, {
      onSuccess: ({ projectItemCount }) => {
        if (typeof recordOracleDiagnostic === "function") {
          recordOracleDiagnostic("info", "LABEL_RECONCILIATION_VERIFIED", { projectItemCount });
        }
      },
      onError: (error) => {
        if (typeof recordOracleDiagnostic === "function") {
        ...
        announce(`Blocky Studios could not reconcile the dropped replay label. ${error.message}`);
      },
    });
  }
    ...
    this.destroyPromise = (async () => {
      try {
        ...
        if (this.activeFileOperationRequestId && nativeDragAddon && typeof nativeDragAddon.cancelFileOperation === "function") {
          await run("native file operation", () => traceNativeCall("cancelFileOperation", () => nativeDragAddon.cancelFileOperation(this.activeFileOperationRequestId), { requestId: this.activeFileOperationRequestId }));
        }
        this.activeFileOperationRequestId = null;
        if (nativeDragAddon && typeof nativeDragAddon.stopDirectoryWatch === "function") {
          await run("native directory watch", () => traceNativeCall("stopDirectoryWatch", () => nativeDragAddon.stopDirectoryWatch()));
        }
        this.nativeLifecycleStarted = false;
        this.nativeWatchRoots = [];
        ...
        if (this.replayMedia) await run("replay media", () => this.replayMedia.destroy());
...
class OracleDedicatedPanelMount {
  constructor(options = {}) {
    ...
    if (!this.kind || !this.shell || !this.controller || !this.panelDom || !this.blueprint) {
      throw new Error("Blocky Studios could not create a complete dedicated panel mount.");
    }
    ...
    this.announceTimer = null;
    this.uiRuntime = oracleWindow.oracleUiRuntime || null;
    this.viewportHandle = this.uiRuntime && typeof this.uiRuntime.mount === "function"
      ? this.uiRuntime.mount(this.root, { panelId: this.panelId, visible: true })
      : null;
  ...
  start() {
    ...
    if (this.uiRuntime && typeof this.uiRuntime.captureAfterLayout === "function") {
      this.uiRuntime.captureAfterLayout(this.root, "workspace-ready-settled");
    }
    return this;
  }
  ...
  showToast(message, kind = "info") {
    ...
    toast.textContent = String(message || "Blocky Studios completed the action.").slice(0, 2048);
  ...
  startReplays() {
    ...
    this.internalViewerAdapter = new replayViewerApi.HtmlVideoReplayAdapter(
      this.elements.replayViewerMedia,
      { mediaPipeline: controller.replayMedia },
    );
    this.viewer = new replayViewerApi.ReplayViewerController({
      root: this.elements.replayViewerTray,
      title: this.elements.replayViewerTitle,
      ...
      adapter: this.internalViewerAdapter,
      onOpenSourceMonitor: (replay) => controller.openInSourceMonitor(replay),
      onUsage: (value) => {
    ...
    this.view = new ReplayGridView(this.elements, {
      ...
      onThumbnailNeeded: (replay) => controller.ensureReplayThumbnail(replay && replay.id),
      getReplayById: (id) => controller.store && controller.store.getById(id),
      getCollectionById: (id) => controller.store && controller.store.state && controller.store.state.collectionsById[id],
      getGridColumns: () => this.gridScale ? this.gridScale.effectiveColumns : 1,
      virtualizer: new replayLibraryApi.ReplayVirtualWindow(),
      nativeAddon: nativeDragAddon,
      nativeAddonDiagnostic: nativeDragAddonDiagnostic,
  ...
  startCurves() {
    ...
    this.workspace = new oracleWindow.OracleCurvesWorkspace.CurvesWorkspaceController(this.elements, {
      ...
      onAnnounce: (message) => this.announce(message),
      viewportService: this.uiRuntime,
      viewportRoot: this.root,
      viewportPanelId: this.panelId,
      fontReady: this.document.fonts && this.document.fonts.ready,
      document: this.document,
    });
  ...
  startQuickApply() {
    ...
    if (!controller.quickApplyAdapterCoordinator || !controller.quickApplyStateStore || !controller.quickApplyRecipeStore) {
      throw new Error("Blocky Studios Quick Apply shared services are not ready.");
    }
    this.promptDialog = new CurvePresetDialogController(this.elements);
    ...
    this.workspace = new controller.quickApplyWorkspaceApi.QuickApplyWorkspaceController(this.elements, {
      ...
      onRequestCloseTopLayer: () => this.announce("Blocky Studios Quick Apply remains open in its dedicated panel."),
      onToast: (message, kind) => this.showToast(message, kind),
      onAnnounce: (message) => this.announce(message),
      document: this.document,
    });
  ...
  setVisible(value) {
    ...
    if (this.viewportHandle && typeof this.viewportHandle.setVisible === "function") {
      this.viewportHandle.setVisible(next);
    }
    if (this.kind === "curves" || this.kind === "quick-apply") {
      if (this.workspace) {
        if (typeof this.workspace.setLifecycle === "function") this.workspace.setLifecycle({ visible: next, active: next });
        else {
  ...
  destroy(_options = {}) {
    ...
    this.destroyPromise = (async () => {
      ...
      if (this.viewportHandle) await run("viewport runtime", () => this.viewportHandle.destroy());
...
function recordOraclePerformanceMeasurement(label, startedAt, outcome = "resolved") {
  recordOracleDiagnostic(outcome === "resolved" ? "info" : "warn", "PERFORMANCE_MEASUREMENT", {
    label: String(label || "Blocky Studios operation"),
...
function formatReplaySpecs(replay) {
  const resolution = normalizeReplayResolution(replay).replace(/\s*×\s*/g, "x");
  const fps = normalizeReplayFps(replay);
  const fpsLabel = fps === null ? "FPS unavailable" : `${Number(fps.toFixed(3))} FPS`;
  return resolution ? `${resolution} - ${fpsLabel}` : fpsLabel;
}
...
function replayRequiresRelink(replay) {
  return Boolean(replay && (replay.missingState === "missing" || replay.missingState === "unknown"));
}
...
function replayCardSignature(replay) {
  ...
  return [
    ...
    String(replay.fps ?? ""),
    replay.timecode || "",
    JSON.stringify(replay.collectionIds || []),
...
function createReplayCollectionChip(collection) {
  const chip = document.createElement("span");
  chip.className = "replay-collection-chip";
  chip.title = String(collection && collection.name || "Collection");
  if (collection && collection.color) {
    chip.style.setProperty("--replay-collection-color", String(collection.color).slice(0, 32));
  }
  const dot = document.createElement("span");
  dot.className = "replay-collection-chip__dot";
  dot.setAttribute("aria-hidden", "true");
  const label = document.createElement("span");
  label.textContent = String(collection && collection.name || "Collection");
  chip.append(dot, label);
  return chip;
}

function createReplayTagChip(value) {
  const chip = document.createElement("span");
  chip.className = "replay-tag-chip";
  chip.textContent = String(value || "");
  chip.title = chip.textContent;
  return chip;
}

function createReplayTagOverflow(count) {
  const chip = document.createElement("span");
  chip.className = "replay-tag-overflow";
  chip.textContent = `+${Math.max(0, Number(count) || 0)}`;
  chip.setAttribute("aria-label", `${Math.max(0, Number(count) || 0)} more tags`);
  return chip;
}
...
function getReplayCanonicalMediaPath(replay, logDetails = false) {
  ...
  if (NATIVE_DRAG_DEBUG && logDetails) {
    console.log("[Blocky Studios Native Drag][Path Resolution]", {
...
function validateImportFilePath(filepath) {
  ...
  if (!importPath || !isAbsoluteLocalPath(importPath)) {
    throw new UserFacingError(
      "Blocky Studios import requires an absolute local file path",
      false,
      "NO_REPLAY_PATH",
...
function normalizeError(error) {
  ...
  if (typeof recordOracleDiagnostic === "function") {
    recordOracleDiagnostic("error", error && error.code || "ORACLE_OPERATION_FAILED", {
      message: typeof oracleErrorMessage === "function"
        ? oracleErrorMessage(error)
        : "An unexpected Blocky Studios operation failed.",
    });
  }
  return new UserFacingError(
    error && error.message ? error.message : "Unexpected Blocky Studios error",
    false,
...
async function verifyBlockyStudiosFonts(packagedFonts) {
  const faces = [
    { id: "samsungSharpSansRegular", family: "Samsung Sharp Sans", weight: "400" },
    { id: "samsungSharpSansMedium", family: "Samsung Sharp Sans", weight: "500" },
    { id: "samsungSharpSansBold", family: "Samsung Sharp Sans", weight: "700" },
  ];
  ...
  const registrationReady = Boolean(
    ...
    packagedFonts.sessionVisible === false &&
    packagedFonts.registrationFlags === "FR_PRIVATE" &&
    packagedFonts.registeredFileCount === faces.length &&
    faces.every((face) => {
      const item = registeredItems.get(face.id);
      return item && String(item.familyName || "") === face.family;
    }),
  );
  const fontSet = document && document.fonts;
  const documentFontsSupported = Boolean(
    fontSet && typeof fontSet.load === "function" && typeof fontSet.check === "function",
  );
  const documentFontResults = new Map();
  if (documentFontsSupported) {
    const loadResults = await Promise.all(faces.map(async (face) => {
      try {
        await Promise.race([
          fontSet.load(`${face.weight} 16px "${face.family}"`, "Blocky Studios 0123456789"),
          delay(1500).then(() => { throw new Error("Font loading timed out."); }),
        ]);
        return [
          face.id,
          fontSet.check(
            `${face.weight} 16px "${face.family}"`,
            "Blocky Studios 0123456789",
          ) === true,
        ];
      } catch (error) {
        return [face.id, false];
      }
    }));
    for (const [id, loaded] of loadResults) documentFontResults.set(id, loaded);
    if (fontSet.ready && typeof fontSet.ready.then === "function") {
      try { await Promise.race([fontSet.ready, delay(1500)]); } catch (error) { /* Measurement remains authoritative. */ }
    }
  }
  const results = [];
  for (const face of faces) {
    const renderProbe = await measureFontRendering(face.family, face.weight);
    const registeredItem = registeredItems.get(face.id);
    results.push({
      id: face.id,
      family: face.family,
      weight: face.weight,
      registered: Boolean(
        registeredItem && String(registeredItem.familyName || "") === face.family,
      ),
      rendered: renderProbe.rendered,
      fontWidth: renderProbe.fontWidth,
      fallbackWidth: renderProbe.fallbackWidth,
      documentFontLoaded: documentFontsSupported
        ? documentFontResults.get(face.id) === true
        : null,
    });
  }
  ...
  const documentFacesReady = !documentFontsSupported || faces.every(
    (face) => documentFontResults.get(face.id) === true,
  );
  if (registrationReady && documentFacesReady && failed.length === 0) {
  ...
  // If the exact product family is already installed, keep it as a compatible
  // rendering path while preserving the packaged registration diagnostic.
  const installedCompatibilityReady = results.every((result) => result.rendered);
  if (installedCompatibilityReady) {
    ...
    if (typeof recordOracleDiagnostic === "function") {
      recordOracleDiagnostic("warn", "INSTALLED_COMPATIBILITY_FALLBACK", {
        ...
        installedFamily: "Samsung Sharp Sans",
      });
    }
...
async function measureBlockyStudiosFontRendering() {
  return measureFontRendering("Samsung Sharp Sans", "400");
}

async function measureFontRendering(fontFamily, fontWeight = "400") {
  if (!document.body || typeof document.createElement !== "function") {
    return { rendered: false, fontWidth: 0, fallbackWidth: 0 };
  }
  ...
  for (const probe of [fontProbe, fallbackProbe]) {
  ...
  fontProbe.style.fontFamily = `"${fontFamily}"`;
  fallbackProbe.style.fontFamily = `"Blocky Studios Host Fallback ${Date.now()}"`;
  ...
  try {
    for (attempts = 1; attempts <= 4; attempts += 1) {
      await delay(100);
      const fontRectWidth = fontProbe.getBoundingClientRect().width;
      const fallbackRectWidth = fallbackProbe.getBoundingClientRect().width;
      fontWidth = Math.max(fontRectWidth, fontProbe.scrollWidth || 0);
      fallbackWidth = Math.max(fallbackRectWidth, fallbackProbe.scrollWidth || 0);
      if (
        Number.isFinite(fontWidth) &&
        fontWidth > 0 &&
        Number.isFinite(fallbackWidth) &&
        fallbackWidth > 0 &&
        Math.abs(fontWidth - fallbackWidth) >= 0.5
      ) {
        break;
      }
    }
...
function setOracleStartupState(state, error = null) {
  ...
  if (title) title.textContent = failed ? "Blocky Studios could not start" : "Loading cinematic tools";
  if (message) {
    const detail = error && error.message ? String(error.message).trim().slice(0, 500) : "";
    message.textContent = failed
      ? `${detail || "The workspace bootstrap failed."} Reload Blocky Studios in UDT after checking the first UXP Console exception.`
...
function createOracleBootstrapNavigationShell() {
  const shellApi = /** @type {any} */ (typeof oracleWindow !== "undefined" ? oracleWindow : window).OracleOverdriveShell;
  if (!shellApi || typeof shellApi.OracleShellController !== "function") return null;
  if (!document || typeof document.querySelector !== "function") return null;
  const root = document.querySelector('[data-oracle-panel-root="oraclePanel"]');
  const elements = {
    navigationToggle: document.getElementById("navigationToggle"),
    navigationBackdrop: document.getElementById("navigationBackdrop"),
    navigationDrawer: document.getElementById("navigationDrawer"),
  };
  if (!root || Object.values(elements).some((element) => !element)) return null;
  const shell = new shellApi.OracleShellController(elements, {
    root,
    document,
...
function destroyOracleBootstrapNavigationShell(bootstrapState) {
  const shell = bootstrapState && bootstrapState.navigationShell;
  if (!shell) return null;
  const route = String(shell.route || "replays");
  bootstrapState.navigationShell = null;
  try {
    shell.destroy();
  } catch (error) {
    if (typeof recordOracleDiagnostic === "function") {
      recordOracleDiagnostic("warn", "BOOTSTRAP_NAVIGATION_TEARDOWN_FAILED", {
        message: typeof oracleErrorMessage === "function"
          ? oracleErrorMessage(error)
          : "Bootstrap navigation teardown failed.",
      });
    }
  }
  return route;
...
function retireOraclePanelBootstrap(bootstrapState) {
  ...
  destroyOracleBootstrapNavigationShell(bootstrapState);
...
async function initializeOraclePanel(bootstrapState = null) {
  ...
  try {
    ...
    if (bootstrapState && !bootstrapState.navigationShell) {
      bootstrapState.navigationShell = createOracleBootstrapNavigationShell();
    }
    const nativeLoadResult = await nativeDragAddonLoadPromise;
    if (!isCurrentOraclePanelBootstrap(bootstrapState)) return;
    try {
      await verifyBlockyStudiosFonts(
      ...
      if (typeof recordOracleDiagnostic === "function") {
        ...
        console.warn("[Blocky Studios][FONT_VERIFIER_UNAVAILABLE] Packaged font verification was unavailable.");
      }
    }
    if (!isCurrentOraclePanelBootstrap(bootstrapState)) return;
    await retireExistingOraclePanelControllers();
    if (!isCurrentOraclePanelBootstrap(bootstrapState)) return;
    const bootstrapRoute = destroyOracleBootstrapNavigationShell(bootstrapState);
    controller = new OraclePanelController();
    if (bootstrapRoute && controller.shell && typeof controller.shell.setRoute === "function") {
      controller.shell.setRoute(bootstrapRoute, false);
    }
    ...
    const readyUiRuntime = /** @type {any} */ (
      typeof oracleWindow !== "undefined" ? oracleWindow : window
    ).oracleUiRuntime;
    const readyRoot = controller.mainRoot || document.querySelector('[data-oracle-panel-root="oraclePanel"]');
    if (readyUiRuntime && typeof readyUiRuntime.captureAfterLayout === "function" && readyRoot) {
      readyUiRuntime.captureAfterLayout(readyRoot, "controller-ready-settled");
    }
    if (typeof recordOracleDiagnostic === "function") {
    ...
    if (typeof reportOracleCritical === "function") {
      ...
      console.error("[Blocky Studios][PANEL_INITIALIZATION_FAILED] Blocky Studios panel initialization failed.");
    }
    await destroyOracleBootstrapController(controller);
...
function captureOracleM7PanelBlueprints() {
  ...
  if (!panelDom || typeof panelDom.capturePristineBlueprint !== "function") {
    return {
      blueprints: null,
      error: new Error("the Blocky Studios root-scoped panel DOM module did not load."),
    };
  }
  try {
    return {
...
function renderOracleM7EmergencyPanelState(rootValue, panelId, error) {
  ...
  try {
    ...
    kicker.textContent = "Blocky Studios workspace";
    const title = document.createElement("h2");
    title.textContent = "Blocky Studios could not start";
    const message = document.createElement("p");
    const detail = String(error && error.message ? error.message : error || "The panel bootstrap failed.").slice(0, 500);
    message.textContent = `${detail} Reload Blocky Studios in UDT after checking the first UXP Console exception.`;
    ...
    if (typeof reportOracleCritical === "function") {
      ...
      console.error("[Blocky Studios][EMERGENCY_PANEL_STATE_FAILED] The emergency panel state could not be rendered.");
    }
    return false;
  }
}

const ORACLE_MAIN_PANEL_SELECTOR =
  '[data-oracle-panel-root="oraclePanel"][data-oracle-panel-kind="main"]';

function resolveOracleMainPanelShell(documentRef = document) {
  if (!documentRef || typeof documentRef.querySelector !== "function") return null;
  return documentRef.querySelector(ORACLE_MAIN_PANEL_SELECTOR);
}
...
class OracleM7PanelHostRegistry {
  constructor() {
    ...
    this.mainLifecycleRoot = null;
    this.mainPanelVisible = false;
    this.mainVisibilityGeneration = 0;
    this.mainViewportHandle = null;
    this.destroyed = false;
    this.destroyPromise = null;
  }
  ...
  setMainPanelVisible(value) {
    ...
    const next = Boolean(value);
    this.mainPanelVisible = next;
    if (this.mainViewportHandle && typeof this.mainViewportHandle.setVisible === "function") {
      this.mainViewportHandle.setVisible(next);
    }
  ...
  panelConfig(panelId) {
    const configs = {
      oracleReplaysPanel: { kind: "replays", kicker: "Replay library", title: "Blocky Studios Replays" },
      oracleCurvesPanel: { kind: "curves", kicker: "Premiere keyframes", title: "Blocky Studios Curves" },
      oracleQuickApplyPanel: { kind: "quick-apply", kicker: "Premiere effects", title: "Blocky Studios Quick Apply" },
    };
    return configs[panelId] || null;
  }

  attachMainPanelShell(rootValue) {
    if (this.destroyed) throw new Error("the Blocky Studios multi-panel runtime is shutting down.");
    const panelDom = oracleWindow.OraclePanelDom;
    if (!panelDom || typeof panelDom.resolvePanelRoot !== "function") {
      throw new Error("the Blocky Studios root-scoped panel DOM module is unavailable.");
    }
    let root = panelDom.resolvePanelRoot(rootValue);
    if (root && root.body && typeof root.body.appendChild === "function") root = root.body;
    if (!root || typeof root.appendChild !== "function") {
      throw new Error("Premiere did not provide an appendable main-panel root.");
    }
    const shell = resolveOracleMainPanelShell(document);
    if (!shell || typeof shell !== "object") {
      throw new Error("the Blocky Studios canonical main-panel shell is unavailable.");
    }
    if (root !== shell && typeof shell.contains === "function" && shell.contains(root)) {
      throw new Error("Premiere supplied a main-panel root inside Blocky Studios' own shell.");
    }
    const rootChanged = this.mainLifecycleRoot !== root;
    const moved = root !== shell && shell.parentNode !== root;
    if (moved) root.appendChild(shell);
    this.mainLifecycleRoot = root;
    const uiRuntime = oracleWindow.oracleUiRuntime;
    if (uiRuntime && typeof uiRuntime.mount === "function") {
      this.mainViewportHandle = uiRuntime.mount(shell, {
        panelId: "oraclePanel",
        visible: this.mainPanelVisible,
      });
    }
    if (moved || rootChanged) {
      recordOracleDiagnostic("info", "MAIN_PANEL_ROOT_ATTACHED", {
        rootId: oracleM7RootId(root),
        rootTag: String(root && root.tagName || ""),
        rootChildCount: Number(root && root.children && root.children.length || 0),
        moved,
      });
    }
    return { root, shell, moved };
  }
  ...
  ensureRecord(panelId, rootValue) {
    ...
    if (existing) {
      if (existing.panelId !== panelId) {
        ...
        const conflict = Object.assign(
          new Error(
            `Premiere dispatched ${panelId} to a root already owned by ${existing.panelId}. Reload Blocky Studios in UDT and inspect the M7 panel-hook sequence.`,
          ),
    ...
    if (!config) throw new Error(`Unsupported Blocky Studios panel: ${panelId}`);
    const shell = panelDom.createDedicatedPanelShell(document, {
      ...
      title: config.title,
      message: "Connecting this workspace to the Blocky Studios shared Premiere runtime.",
    });
    const record = {
      panelId,
      kind: config.kind,
  ...
  async waitForController() {
    for (let attempt = 0; attempt < 240; attempt += 1) {
      ...
      if (bootstrap && bootstrap.promise) {
        throw new Error("the Blocky Studios shared application runtime did not finish starting.");
      }
      await delay(25);
    }
    throw new Error("the Blocky Studios shared application runtime did not become ready.");
  }
  ...
  blueprintFor(record) {
    ...
    if (!blueprints) throw new Error("the Blocky Studios dedicated workspace blueprints are unavailable.");
  ...
  prepare(panelId, rootValue) {
    if (panelId === "oraclePanel") {
      try {
        this.attachMainPanelShell(rootValue);
      } catch (error) {
        renderOracleM7EmergencyPanelState(rootValue, panelId, error);
        recordOracleDiagnostic("error", "MAIN_PANEL_ROOT_ATTACH_FAILED", {
          hook: "create",
          message: oracleErrorMessage(error),
        });
      }
  ...
  destroyAll() {
    ...
    this.destroyPromise = (async () => {
      ...
      this.records.clear();
      if (this.mainViewportHandle && typeof this.mainViewportHandle.destroy === "function") {
        try { this.mainViewportHandle.destroy(); } catch (error) {
          recordOracleDiagnostic("warn", "MAIN_VIEWPORT_TEARDOWN_FAILED", {
            message: oracleErrorMessage(error),
          });
        }
      }
...
function oracleUiHealthContext() {
  const controller = Reflect.get(window, ORACLE_PANEL_CONTROLLER_KEY);
  const dedicatedMounts = controller && controller.dedicatedMounts
    ? Array.from(controller.dedicatedMounts)
    : [];
  const graphWorkspaces = [];
  if (controller && controller.curvesWorkspace) graphWorkspaces.push(controller.curvesWorkspace);
  for (const mount of dedicatedMounts) {
    if (mount && mount.kind === "curves" && mount.workspace) graphWorkspaces.push(mount.workspace);
  }
  const uniqueGraphWorkspaces = Array.from(new Set(graphWorkspaces));
  const activeGraphWorkspaces = uniqueGraphWorkspaces.filter((workspace) => (
    workspace && workspace.destroyed !== true && workspace.visible !== false && workspace.active !== false
  ));
  const pendingGraphFrames = activeGraphWorkspaces.filter((workspace) => workspace.renderHandle !== null).length;

  const diagnosticRecords = oracleDiagnostics && typeof oracleDiagnostics.snapshot === "function"
    ? oracleDiagnostics.snapshot({ limit: 500 })
    : [];
  const retainedRenderMeasurements = diagnosticRecords.filter((entry) => (
    entry &&
    entry.code === "PERFORMANCE_MEASUREMENT" &&
    entry.details &&
    /render/i.test(String(entry.details.label || ""))
  ));
  const retainedRenderLabels = {};
  for (const entry of retainedRenderMeasurements) {
    const label = String(entry.details.label || "render").slice(0, 120);
    retainedRenderLabels[label] = (retainedRenderLabels[label] || 0) + 1;
  }

  let services = null;
  try { services = oracleM7PanelHostRegistry.serviceSnapshot(controller); } catch (error) { services = null; }
  let platformTelemetry = null;
  try {
    platformTelemetry = oraclePlatformTelemetry && typeof oraclePlatformTelemetry.audit === "function"
      ? oraclePlatformTelemetry.audit()
      : null;
  } catch (error) { platformTelemetry = null; }
  const nativeErrorCode = String(nativeDragAddonDiagnostic && nativeDragAddonDiagnostic.errorCode || "");
  return {
    available: true,
    currentRoute: String(controller && controller.shell && controller.shell.route || ""),
    nativeAddon: {
      available: Boolean(nativeDragAddon),
      status: nativeDragAddon
        ? "ready"
        : nativeErrorCode
          ? "unavailable"
          : "loading-or-unobserved",
      errorCode: nativeErrorCode,
      bootstrapTimedOut: nativeAddonBootstrapTimedOut === true,
    },
    graphAnimation: {
      available: true,
      schedulerModel: "one-shot-requestAnimationFrame",
      workspaceCount: uniqueGraphWorkspaces.length,
      activeWorkspaceCount: activeGraphWorkspaces.length,
      activeFrameCount: pendingGraphFrames,
      multipleActiveFrames: pendingGraphFrames > 1,
      continuousLoopCount: 0,
    },
    renderCount: oracleProfilerRestores.length > 0
      ? {
          available: true,
          bounded: true,
          scope: "retained opt-in profiler render measurements",
          value: retainedRenderMeasurements.length,
          labels: retainedRenderLabels,
        }
      : {
          available: false,
         reason: "OPT_IN_PERFORMANCE_PROFILER_INACTIVE",
        },
    platformTelemetry: platformTelemetry || {
      available: false,
      reason: "PLATFORM_TELEMETRY_UNAVAILABLE",
    },
    services,
  };
}

if (oracleWindow.oracleUiRuntime && typeof oracleWindow.oracleUiRuntime.setHealthContextProvider === "function") {
  oracleWindow.oracleUiRuntime.setHealthContextProvider(oracleUiHealthContext);
}
  ...
  return Promise.resolve().then(() => {
    ...
    if (!plugin) {
      throw oracleM8CommandError(
        ...
        "Blocky Studios is not present in Premiere's enabled UXP plugin list.",
      );
    }
    if (plugin.enabled !== true) {
      throw oracleM8CommandError(
        "ORACLE_PLUGIN_DISABLED",
        "Blocky Studios is disabled in Premiere's UXP plugin manager.",
      );
    }
    ...
    try {
      ...
      throw oracleM8CommandError(
        "SHOW_PANEL_THROW",
        `Premiere could not reveal Blocky Studios Quick Apply: ${oracleM8HostMessage(error, "unknown host error")}`,
        error,
      );
    }
    if (typeof result === "string") {
      throw oracleM8CommandError(
        "SHOW_PANEL_REJECTED",
        `Premiere rejected Blocky Studios Quick Apply: ${oracleM8HostMessage(result, "panel reveal was rejected")}`,
      );
    }
    if (!result || typeof result.then !== "function") {
      throw oracleM8CommandError(
    ...
    return Promise.resolve(result).then((value) => {
      if (typeof value === "string") {
        throw oracleM8CommandError(
          "SHOW_PANEL_REJECTED",
          `Premiere rejected Blocky Studios Quick Apply: ${oracleM8HostMessage(value, "panel reveal was rejected")}`,
        );
      }
      return undefined;
    }, (error) => {
    ...
    throw oracleM8CommandError(
      ...
      `Blocky Studios Quick Apply could not be opened: ${oracleM8HostMessage(error, "unknown command error")}`,
      error,
    );
  });
}
  ...
  void operation.catch((error) => {
    try {
      const diagnostic = {
        ...
        message: oracleM8HostMessage(error, "Blocky Studios Quick Apply could not be opened."),
      };
      if (typeof recordOracleDiagnostic === "function") {
        recordOracleDiagnostic("error", diagnostic.code, { message: diagnostic.message });
      }
      if (logger && typeof logger.error === "function") {
        logger.error("[Blocky Studios M8][oracleQuickApplyCommand] Panel reveal failed.", diagnostic);
      }
    } catch (loggingError) {
  ...
  entrypoints.setup({
    plugin: {
      ...
      async destroy() {
        ...
        const buttonPaintBridge = oracleWindow.oracleButtonPaintBridge;
        if (buttonPaintBridge && typeof buttonPaintBridge.destroy === "function") {
          buttonPaintBridge.destroy();
        }
